...
`node tests/raw-source-properties.mjs` covers the shared raw-source model — known SHA-256 vectors, the manifest's detection of add/delete/rename/byte-change, the five model states, root aliases and hardlinks, snapshot bytes, injected read races, and address resolution. It builds fixtures in a temp directory, never in the repository. Its output ends with `nonregular=`, `rootalias=` and `hardlink=`, which record which fixture form each branch used. There is no fallback left: every link fixture targets a DIRECTORY, which a Windows junction covers without privilege and a POSIX symlink covers by ignoring the type argument, so the same branches run on every host and a creation failure raises instead of skipping. Identity checks are BigInt end to end — Number holds 53 exact bits and NTFS inodes are 64.
...
| `bundle.manifest` (+`.sha256`) | Phase 0 시점 21개 동작 결정 파일의 SHA-256 (git blob 기준). 재생성: `bash tests/make-manifest.sh` (현재 58개 — VERSION·`.weavedoc/.gitattributes`·공유 scanner/state adapters·`PARSER-MODEL.md`·versioned `schemas/`·templates 포함). 생성기는 **fail-closed**: 저장소가 없거나 필수 경로가 빠지면 빈 매니페스트에 rc 0이 아니라 **rc 2로 거부**한다(v0.5.18) |
