...
acct_interview_output_is_pure_ascii() {
  ...
  # Two arrays, six questions: AskUserQuestion takes at most four questions per call and the
  # interview asks six, so a single-array payload is unusable no matter how correct its bytes are.
  local na nq
  na=$(printf '%s\n' "$OUT" | grep -c '^\[')
  nq=$(printf '%s\n' "$OUT" | grep -o '"question":"' | grep -c .)
  [ "$na" = "2" ] || { bad "interview printed $na array(s), not 2 (AskUserQuestion caps a call at 4 questions; the interview asks 6)"; return; }
  [ "$nq" = "6" ] || { bad "interview printed $nq question(s), not 6"; return; }
  ok
}
...
EOF
  ...
  [ "$n" = "6" ] || { bad "the payload split found $n question(s), not 6 — the parse is broken, so nothing was compared"; return; }
