...
But "we couldn't catch it, oh well" is **not** acceptable as a posture. A miss is a *defect to fix*, not a cost to accept. Recall is an **investment**: hunt on purpose (not by luck), cross-check every structured fact (numbers·dates·amounts·obligations) exhaustively, check **at the moment of citation** (about to cite A — do the same-topic materials disagree?), flag a contestable fact cited from a *single* material when siblings exist, and re-run detection as the source grows so a late-arriving material re-opens a settled question. Honesty about the residual hard tail comes only *after* that effort — never as an excuse to do less.

**5.2 The AI detects; the human rules; the machine keeps the ledger.** Picking a winner is conferring truth — the human's job, with no exception: no authority order, no date order, no recency rule decides for them. A detected disagreement is *recorded* (an open entry in the conflict ledger — the cards themselves never wear it), the record blocks shipping until the human rules, and the ruling is *applied* (the card edited in place, evidence merged, claims split, or everything rejected) — after which the entry is deleted, because a resolved disagreement is not history the source carries. A claim rejected once that arrives again is compared fresh: nothing is suppressed by having lost before.

**5.3 Keeping both is a split, and the split is the record.** When both sides are genuinely right, there is always a hidden axis — time, viewpoint, definition, scope, or at last resort plain attribution ("Source A reports X; Source B reports Y") — and the resolution is to *write that axis into the claims* so they no longer collide. That lifts the clash from the object level (*what's true*) to the meta level (*who said what / when / under which definition*), and each rewritten claim is itself a single coherent truth. The split is the human's ruling like any other — the machine may **never elect** it, because deciding "both stand" is still a truth judgment. And no authorization record is kept beyond the cards themselves: the rewritten claims *are* the answer. This closes an escape hatch: otherwise a lazy "both sides" sentence could launder any conflict past the gate.

The invariant underneath all of this: **the source may never hold a bare contradiction.** Every disagreement ends applied (a single truth per claim, axes written in where both stand) or stays an open entry that blocks shipping. Contamination — a floating X-and-not-X — is made unreachable.
...
- **It parks the decision in a file and reports "done — check the file"** → so the closing report carries the open items *themselves*: every conflict, open question, Human-queue entry, fidelity violation and open gap stated in the message with what's needed from the human, the file path as reference only. A pointer whose substance lives only on disk re-creates the no-reviewable-surface failure at the handoff — the human has to open files just to learn what is being asked of them. And the list is *mechanical* (`weavedoc status --open`), because a report composed from memory is a fifth reader of the same ledgers — which is exactly how the first version of that command came to call a blocking gap "nothing is waiting".
...
- **Target:** Claude Code and Codex terminals.
- **Form:** one byte-identical per-step skill set in each harness discovery root (`.claude/skills/weavedoc-*`, `.agents/skills/weavedoc-*`) + a shared light runtime (`.weavedoc/`: `config.yaml`, templates, `schema`, the read protocol `READ.md`, and a dependency-free Node toolbelt `bin/weavedoc.mjs` — validate · pull · census · reindex · retag · gaps). Harness differences are runtime adapters, not workflow forks. Invoked by natural language, not slash commands.
- **The mine serves consumers outside the pipeline too.** Creative work (lyrics, fiction) is out of scope for plan→write — invention can't pass the gate, by design — but it still *reads* the mine. That boundary has three doors: **pull** (protocol-correct read out), **gather** (declared results back in, gated), and the read protocol (`READ.md`, pointed to from the project's `CLAUDE.md` and `AGENTS.md`) for everything in between. A misread cannot poison the mine — write-side safety is mechanical — it can only make the outside output wrong; and if that output re-enters via gather, map's conflict hunt is the backstop.
- **One engine, per-project knobs.** The same machinery serves a research log and a contract; a few declared settings differ: `fidelity.completeness` (is omission a violation?), `conflicts.detection` depth, `authority` (**who decides** — `strict` | `standard` | `delegated`, orthogonal to how hard the machine checks; each skill carries its row in its `## Authority` section, and a document may override in its plan). Set once, at the declaration moment.
...
- **The unit of work is a markdown file on disk.** Not tied to a tracker or API — a board just reads these files. Delete `.weavedoc/` and the installed harness skill roots, and the tool is gone while the documents and materials stand intact.
