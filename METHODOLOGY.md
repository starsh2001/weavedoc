...
- **Form:** per-step skills (`.claude/skills/weavedoc-*`) + a light runtime (`.weavedoc/`: `config.yaml`, templates, `schema`, the read protocol `READ.md`, and a dependency-free Node toolbelt `bin/weavedoc.mjs` — validate · pull · census · reindex · retag · gaps). Invoked by natural language, not slash commands.
