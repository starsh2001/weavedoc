...
| 5 | **`config.paths`에 MSYS 형식 절대경로** | 고칠 수 없다 — #3과 **같은 경계**다. MSYS의 `/tmp`는 `C:\tmp`가 아니라 `C:\Users\<u>\AppData\Local\Temp`이고(실측), 정적 규칙으로 옮길 수 없다. `cygpath` 없이는 MSYS 밖의 누구도 모르고, PowerShell에서 부른 같은 런타임에게 `/tmp`는 정말로 `D:\tmp`다. bash는 자신이 MSYS 프로그램이라 해석하고, 배포되는 Node는 네이티브라 `CFG-PATH-MISSING`을 낸다. **Windows에서 config에 `/c/...`를 손으로 적은 경우에만** 나타나며 판정·digest·id에는 닿지 않는다(경로 하나가 없다고 말할 뿐이다). 2026-08-05 v0.4.0 태그 런이 찾았다 — 케이스 `pass_paths_absolute`가 경로를 MSYS 형식으로 만들고 있었고, 그건 그 케이스가 고정하려던 약속(*절대경로가 먹힌다*)이 아니라 **MSYS 경로 번역**을 재고 있었다. Windows에서 네이티브 형식(`pwd -W`)을 쓰도록 고쳤고, 편집이 실제로 반영됐는지 확인하는 무의미-방지 가드를 붙였다 |
...
**덧 (2026-08-05, v0.4.0 태그 런):** 이 케이스가 macOS 레그에서 실패했다 — `id source tags claim status`, 정렬도 아니고 gawk 해시 순서도 아닌 **제3의 순서**. 원인은 코드가 아니라 **채점 환경**이었다. `brew install gawk`는 gnubin 디렉터리도 `awk` 링크도 만들지 않으므로 `awk`는 계속 `/usr/bin/awk`(BWK)였고, **macOS 레그는 처음부터 지원하지 않는 툴체인을 채점하고 있었다.** 도구 자신의 preflight로는 못 잡는다 — BWK awk도 `awk --version`에 0으로 답한다. CI에 `awk → gawk` 심링크를 놓고, **PATH 위의 awk·sed가 정말 GNU인지 확인하는 단계**를 별도로 붙였다(`$GITHUB_PATH`는 다음 단계부터 유효하다). 조용한 BSD 폴백은 이 레그가 막으라고 있는 바로 그것이다. bash 판의 preflight도 같은 구멍이 있지만(문서화한 바닥을 실제로 강제하지 않는다) **거기는 고치지 않는다** — 레거시 참조 런타임에 동작을 더하지 않는다는 규칙이 우선하고, Node 판은 sed/awk를 아예 부르지 않아 대응물이 없다.
