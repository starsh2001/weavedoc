...
`.weavedoc/bin/weavedoc` ships a dependency-free checker (needs **Bash 4+ and GNU awk/sed** — what Git for Windows and Linux provide out of the box; macOS ships Bash 3.2, so install a newer bash there. macOS is a **required CI platform** (promoted 2026-08-04 after the known index-parsing failures resolved in v0.3.4) — Linux, Windows and macOS all gate the release — `pull` uses associative arrays) — **the mechanical floor under the AI fidelity gate:**
