...
- `pull <term>` — protocol-correct mine lookup for consumers *outside* the pipeline (creative sessions, other tools): searches claims+tags (body fallback); every card it lists is canonical (schema v3 — an undecided disagreement lives in conflicts.json and blocks shipping, never wears a card), with `as_of` / derived / plan-stage / retracted-source labels attached. See `.weavedoc/READ.md`; `init` plants byte-identical pointer blocks in `CLAUDE.md` and `AGENTS.md`, and `validate` warns (`CLAUDE-BLOCK-STALE` / `AGENTS-BLOCK-STALE`) when either pointer is no longer the one this bundle ships — it is injected before any file is read, so a stale one primes rather than merely misinforms.
