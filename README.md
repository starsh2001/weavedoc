...
`.weavedoc/bin/weavedoc` ships a dependency-free checker (needs **Bash 4+ and GNU awk/sed** — what Git for Windows and Linux provide out of the box; macOS ships Bash 3.2, so install a newer bash there. macOS support is **best-effort** (decision 2026-08-03): Linux and Windows are the required CI matrix, macOS runs non-blocking on tags with 4 known index-parsing case failures recorded, pending a macOS shell to pin them — `pull` uses associative arrays) — **the mechanical floor under the AI fidelity gate:**
