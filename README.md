...
.weavedoc/bin/weavedoc.mjs  deterministic checks — validate · pull · impact · status · scope ·
                         intake · attest · seal-review · consecrate · gaps · census ·
                         reindex · retag · version · lang · locale · interview · activate
.weavedoc/bin/lib/       the runtime's modules (the behavior lives here, not in the entrypoint)
.weavedoc/VERSION        the version (SemVer, bumped every bundle) — the git tag and the
                         CHANGELOG section heading follow this file; identity is `version`'s
                         fingerprint, not the version number
```
...
WeaveDoc ships the same nine skills for Claude Code and Codex. To use it in a project:

1. Copy `.weavedoc/` and the skill root for your harness: `.claude/skills/weavedoc-*` for Claude Code, `.agents/skills/weavedoc-*` for Codex, or both for a dual-harness repo.
2. Ask Claude Code or Codex: **"weavedoc init"** — it creates the workspace and `.weavedoc/config.yaml`, plants the matching instruction pointers, and wires both hook surfaces.
...
On Codex, project hooks run only in a trusted project. After init, review `/hooks` once and trust the project when prompted; `.codex/hooks.json` then observes the skill's explicit `activate` handshake and applies the same per-session write gate as Claude Code.
...
**Keeping installs in sync.** `node .weavedoc/bin/weavedoc.mjs version` prints three lines: the **version** (`.weavedoc/VERSION`), the **fingerprint** (bin+schema content hash — compare THIS, two installs can share a version while their bin differs), and the **schema version** this runtime reads. Releases add a SemVer tag whose bundle manifest covers every behavior-deciding file. If you evolve the skills/runtime *inside* a project (the testbed pattern), backport here and bump `VERSION` — the runtime once grew two weeks ahead inside a testbed while this repo went stale.

**One version for every bundle.** `VERSION` is SemVer and moves on every bundle, so it is the CHANGELOG section heading and the git tag, and there is no second label to keep in sync. `tests/doccheck.sh` ties `VERSION` to the CHANGELOG's top section on every regression sweep, and the release job refuses to publish a tag that is not `v` + `VERSION`. (Before v0.6.6, `VERSION` held a date stamp and the release version existed only in git tags.)
...
- `validate` — format + truth coherence (schema v3): the version gate first (a non-v3 mine gets ONE answer — which migration path — and no verdict about anything else), the two state files present and parseable (`STATE-MISSING`/`STATE-MALFORMED` — unreadable state never reads as empty), open conflicts block (`CONFLICT-OPEN`), frontmatter/ids, no card wears a v2 state field (`TRUTH-V2-FIELD`), catalog ↔ materials orphans both ways, every truth `source` resolves to a live material (a `retracted` material grounds nothing — re-ground or delete the card), `provenance` enum valid and `derived` truths show their `derived_from` chain, **each truth's body appears verbatim in its source** (the anti-laundering seal), every `required_tags` tag has at least one truth, `index.md` ↔ truth files in sync both ways, `truths/coverage.md` cross-checks, `origin: research` materials carry `url`+`retrieved_at` and their truths are not `provenance: stated`, every `[open]` Human queue entry carries an ownership tag, no `final.md` ships with a non-empty `# Fidelity violations`. Exits non-zero with the list.
- `census` — the mine's authoritative statistics: truth files vs index entries and `coverage records N/M of TOTAL (K legacy-exempt)`, plus the materials behind the shortfall — `→ unmapped:` (no coverage record, no card) and `→ no coverage record, cards exist:` (mapped before coverage; a ruling, not extraction) — the raw total is always shown, because `16/26` and `16/16 (+10 exempt)` describe the same mine and a reader comparing two reports would otherwise see progress that never happened. (No status tallies and no numbering-hole ledger since schema v3: every existing card is canonical, and a hole is the allocator's normal trace of a deletion, never a question.) **`coverage records` counts materials that hold at least one line in `truths/coverage.md`; it is a ledger count, not a completeness warranty** — one recorded element out of fifty still counts the material. "Is everything that should be extracted here?" is a different axis, owned by `weavedoc-gaps` and the truths verify lane. Skills report these numbers, never eye-counts.
...
- `pull <term>` — protocol-correct mine lookup for consumers *outside* the pipeline (creative sessions, other tools): searches claims+tags and card bodies in one pass (index matches first, body-only labelled below); every card it lists is canonical (schema v3 — an undecided disagreement lives in conflicts.json and blocks shipping, never wears a card), with `as_of` / derived / plan-stage / retracted-source labels attached. See `.weavedoc/READ.md`; `init` plants byte-identical pointer blocks in `CLAUDE.md` and `AGENTS.md`, and `validate` warns (`CLAUDE-BLOCK-STALE` / `AGENTS-BLOCK-STALE`) when either pointer is no longer the one this bundle ships — it is injected before any file is read, so a stale one primes rather than merely misinforms.
...
- `scope` — what a verify round still owes, split by evidence class: **verified (digest-bound)** — a `truths/verify-ledger.tsv` row whose sha256 matches the unit's current bytes · **legacy-unbound** — a digest-less v1 record (a material's own `status: verified`, or a markdown `## Verified units` row) that is preserved history but binds no bytes · **stale** — digest mismatch, the unit changed after verification · **failed** · **unverified**. A round owes `unverified + stale + failed`; legacy-unbound is re-verified by risk priority, not wholesale. It also prints the **intake** lane — `declared (digest-bound) · no-source · legacy-unbound · stale · undeclared` — and this is where the intake digests are actually compared, because it is the command that re-reads every original (`validate` and `census` report row presence only, and say so). The verify skill reads its round scope from here rather than deciding it — asked which truths a round owed, a real run answered "all of them" and put five cold reviewers across 264 truths, three rounds deep, when the answer was 40.
- `intake [--no-source] <material-id> <note>` — the **declaration** write path, and the mine's only mechanical gate on what comes IN. It records that something was handed over — a tree digest over every `source.*` in the material folder, how many files it covers, and a note saying how it arrived — as an append-only row in `materials/intake-ledger.tsv` (last row per id wins, so a re-declaration is an append). What it closes: the agent that writes a material also writes the cards citing it, so an **invented** material is structurally perfect — quotes match, coverage closes, seals pass — and every check downstream of a forgeable root only states the forgery more confidently. It does **not** prevent fabrication (an agent can invent a `source.md` and declare it); it takes fabrication off the *default* path and makes it visible: a silent file write becomes a command, a row, and a digest bound to bytes that had to exist. `--no-source` is the ruled exception for a material with no original to bind to — it demands the ruling in the note, binds no bytes, and is counted apart everywhere. `validate` warns `MAT-UNDECLARED` for a material with no row (**a warning, never a block** — every mine predates the ledger, and a check that reddens every project is a check nobody reads); a pre-ledger backlog is bound in place with `--anchor-existing`, one deliberate act by a person.
- `attest <verified|failed> <round> <standard> <id...>` — the verification write path: computes each unit's digest (the one spelling of the hash rule — truth = whole file; material = `converted.md` minus its lifecycle `status:` line), appends sidecar rows (append-only, last row per id wins), and mirrors a readable line into `## Verified units`. All-or-nothing on an unresolvable id; a retracted material is refused (the material lifecycle survives v3 — every truth card that exists is attestable).
...
- (There is no migration command. The v2→v3 migrator retired in 0.6.15, the way the v1 path did before it: **pinned to the last bundle that carried it** — v0.6.14, commit `924e97e`. A v2 mine is refused toward that checkout by the version gate; a v1 mine toward the v0.5.21 bridge (commit `0257167`) first. See [UPGRADING.md](UPGRADING.md).)
- `conflict list | add <entry.json> | remove <cNNN>` — the open-disagreement ledger (`.weavedoc-state/conflicts.json`). Detection is the AI's judgment and arrives already made; the ruling is the human's and never happens here — `add` records a disagreement (payload as a JSON file, prose never rides argv; the id is granted by the allocator so a caller can neither pick nor reuse a number), `remove` records that a ruling was applied elsewhere, by deleting the entry. Every open entry blocks shipping.
...
- `status` — each document's stage and its next step, plus the open Human-queue split (you decide / recommendation ready / machine can just do). `status --open` lists the waiting items themselves — every open conflict · question · Human-queue entry · fidelity violation · gap, one line each, through the same judges validate uses (the register reader, the gate's own body reader, the one Human-queue classifier the counters share) — so a closing report can paste the list instead of re-composing it. `version` — version · fingerprint · schema version. It closes with **where you are and what you could start**: a `phase:` line (mine-building · document-writing, with the documents in flight) and an `available` block naming every action with work ACTUALLY waiting — files in the inbox, materials with no coverage record and no card (the same judgment census prints — a material done with zero cards is not map work), units with no verification record, a document mid-pipeline — each with its reason. It **lists, it never picks**: WeaveDoc has no obligatory order, so choosing among what is available stays the user's, and an action with no reason is simply absent (a block that named every legal skill would name nearly all of them, always). The verify entry is deliberately the WEAKER claim and says so — `scope` owns the verdict that compares digests.
...
- `interview [claude|codex]` — init's fixed setup questions as two ready-made payloads for the active harness (`AskUserQuestion` or `request_user_input`). The output is **entirely ASCII**, every Korean character pre-escaped, because the step it removes is an agent hand-encoding Hangul on the way into a tool call. Claude labels are the config values; Codex strips the exact UI suffix ` (Recommended)` before writing the chosen value. Like `version`/`lang`/`locale` it opens no mine and takes no schema-version gate.
- `activate <weavedoc-skill>` — Codex's explicit session handshake. The command itself is side-effect-free; the project `PostToolUse` hook receives the session id and records the same short-lived ownership lease that Claude Code derives from its `Skill` event.
...
- **`N seal FAILED`** — checked and NOT found. Never folded into `sealed`. (There is no exempt bucket: every v3 card is canonical and owes a seal — `N NOT checked` appears only when a card was never checked at all.)
