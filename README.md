...
.weavedoc/VERSION        the version (SemVer, patch bumped every bundle) — the git tag and the
...
- `pull <term>` — protocol-correct mine lookup for consumers *outside* the pipeline (creative sessions, other tools): searches claims+tags (body fallback); every card it lists is canonical (schema v3 — an undecided disagreement lives in conflicts.json and blocks shipping, never wears a card), with `as_of` / derived / plan-stage / retracted-source labels attached. See `.weavedoc/READ.md`; `init` plants a CLAUDE.md pointer so every session hits the protocol, and `validate` warns (`CLAUDE-BLOCK-STALE`) when that pointer is no longer the one this bundle ships — it is injected before any file is read, so a stale one primes rather than merely misinforms.
...
- `upgrade [--check|--dry-run|--apply]` — the v2→v3 migrator: backup is a CLEAN git worktree (checked, never operated — recovery is `git restore . && rm -rf .weavedoc-state`, printed as words), the transform is total over the v2 status axis (keep+strip · delete · move-to-conflicts.json), blocked items (`unsupported` cards, authorized 병기, citations of leaving cards) stop it before the first write, and the verify is one conservation equation plus an EXACT validate expectation. Machine ledgers travel with a deletion: the verify sidecar loses the row, and a `coverage.md` bullet whose every id left is **rewritten as a skipped entry, never dropped** — the element must stay accounted for or the next map re-extracts what the mine just superseded. It also **backfills the intake ledger**: every material predating that ledger gets a `legacy-unbound` row, so `MAT-UNDECLARED` means "appeared after the ledger and was never declared" rather than "this mine is old" — and because of that, an *already-v3* mine no longer returns immediately from `upgrade`, it checks whether that backfill is still owed. A v1 mine is routed to the pinned bridge runtime v0.5.21 first. **At most one mode token**: `--check --apply` is refused rather than resolved, because the two say opposite things about writing and last-wins is a rule no gate shares (it wrote, measured, until bundle `.34`). See [UPGRADING.md](UPGRADING.md).
