...
- `pull <term>` — protocol-correct mine lookup for consumers *outside* the pipeline (creative sessions, other tools): searches claims+tags (body fallback); every card it lists is canonical (schema v3 — an undecided disagreement lives in conflicts.json and blocks shipping, never wears a card), with `as_of` / derived / plan-stage / retracted-source labels attached. See `.weavedoc/READ.md`; `init` plants a CLAUDE.md pointer so every session hits the protocol.
