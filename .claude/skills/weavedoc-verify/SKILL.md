...
5. **Budget and stop safety.** `config.verify.max_rounds` budgets the **whole cycle** — every cold round of every phase counts against it. Do the arithmetic before promising slack: collection dries in 2–3 rounds measured, and a cycle that found anything spent at least 3 of them (the find + the two dry rounds that end the phase), so the default budget (5) leaves **at most 2 proof rounds** — one spare at `standard` (`repeat: 1`), **zero** at `full` (`repeat: 2`). An earlier version of this paragraph claimed the majority of the budget lands on proof rounds; the 2026-09-22 evaluation caught that as a contradiction the rules themselves compute, and the numbers above are the correction. The budget is a ceiling, not a margin — and the path that exhausts it is narrow: only a finding that **survives the defender at the `config.verify.strength` bar**, or a bar-crossing ticket-downgrade, sends the cycle back to step 2 and resets the count; dismissed findings and sub-bar notes leave the counter alone (step 4's clean-round definition). Budget exhausted with the count still short → set `status: escalated`, take the open issues to the human, and report *how many* clean rounds in a row were reached (`5 rounds, 1/2 clean in a row`) — never as "passed". Do **not** approve.
...
2. **Highlight the machine-alone judgment set** on top of the list: `derived` truths, hedges hardened into assertions, machine-made negative propositions, guideline grade changes — labeled honestly ("리뷰어가 2회 지적, 기계는 '함의됨'으로 유보"). Each carries one line of **what breaks if it's wrong** ("만나이라면 나이표 전체가 한 살 밀림") + the truth link. An `adopted` value stays in step 1's list under its `[제안 채택]` tag and is **not raised here** — the user chose it in a visible exchange, and what it was owed (proof the choice was transcribed faithfully) was paid in place when it was applied (the Authority row above); raising it again is the asking-twice step 6 bans. (0.6.13 taught the Authority row and left this step still naming `adopted` — the two-answers class inside one file, found when the owner reported being re-asked on a 0.7.3 install.)
