...
5. **Severity**: the three grades and their **entry tickets** are defined **once** in the shared engine (reviewers.md, Findings format) — a finding missing its ticket files one grade lower, and that formal test is the defender's DOWNGRADE call; semantic downgrades still go to the Human queue (§4).
