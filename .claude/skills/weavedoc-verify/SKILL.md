...
- **Material**: pass → set frontmatter `status: verified` **and run `bash .weavedoc/bin/weavedoc attest verified <round> <level> <mID>`** — attest computes the digest and writes the sidecar row that makes the verdict digest-bound. The frontmatter flag alone is a v1 signal and reads as `legacy-unbound`. Fail/escalate → stays `converted` (record the failure with `attest failed …` so the round is on the ledger).
