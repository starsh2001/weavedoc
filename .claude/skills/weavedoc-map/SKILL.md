...
7. **Self-audit the cards you just wrote — before the indexes, before the log.** The cold `verify truths` lane grades extraction on five lenses; **the first three are this step's job first.** Verify exists for what this step missed, not as the place the rules live (ruled 2026-08-16: map had no self-audit at all, so every extraction defect was paid for later with a cold panel, and a real mine carried 50 unpassed cards that way).

   It runs **here** and not after step 9, because everything below it — the indexes, the changelog delta, the human confirmation that reads that delta — describes the cards as they stand. Auditing after logging means logging a delta you are about to change.

   Walk each and **write the result down; an unshown check is not done**:

   - **Every body is a verbatim quote.** For each card written this run, find its body text in the named material's `converted.md`. `validate` already checks substring existence — what you check is that the quote is *one contiguous passage* and that the `location` names where it actually sits. A body assembled from lines that sit apart is a passage the source never had.
   - **The claim says what its own body says.** Not narrower, not broader, no added certainty, no re-attribution. The failure mode is a claim that summarizes the body's *first half* and silently drops the rest — a real run shipped a claim covering 5 of a table's 7 rows, and `pull` shows the claim only, so the missing two were invisible to every consumer.
   - **Coverage actually closes.** Re-read the `## m<id>` section you wrote in step 3 against `converted.md`: is every fact-bearing element there as extracted-or-skipped, is each `skipped:` reason still true, does each id mapping hold? This is the manifest's whole purpose — writing it and not reading it back is the same as not writing it.
   - **One card, one fact.** Scan for cards carrying two independent facts joined by a connective. Bundling is not merely untidy: one wrong half drags the valid half through a correction, and a table-shaped card takes every row down with one bad cell (which is why attribute tables split by row).

   **When this run applied a correction** (a `corrects:` material), add one more: the corrected cards must have been **re-grounded in place at the same `tNNN`** — same id, `source`/`location` repointed, body re-copied from the correcting material. If instead you find NEW cards carrying the correction while the old cards still point at the old material, the correction was *appended, not applied*, and the mine now holds a v2-style supersession chain that v3 exists to be rid of. Fix it before reporting. (This is exactly what happened on 2026-08-16, four times in one session, and nothing caught it because the rule lived only in §On-demand correction below.)

8. **Regenerate indexes mechanically.** Run `node .weavedoc/bin/weavedoc.mjs reindex` — it regenerates `truths/index.md` + `truths/tree.md` from frontmatter in one deterministic pass. **Never hand-edit these files** (a real run spent ~45% of its tool calls hand-patching them and corrupted an entry; `validate` now fails on index↔file drift). Mine statistics in your report (truth-file count · index entries · coverage records) come from `node .weavedoc/bin/weavedoc.mjs census` — never from your own counting (a real run reported 191/181 for a mine of 188/178).

9. **Log the run delta.** Append a block to `truths/changelog.md` (create it if absent; format per FORMATS.md): `added:` lines (id + `[provenance]` + one-line claim), `edited:` lines (id + what changed — every in-place update lands here: adoption, re-grounding, split), `removed:` lines (id + why the card was deleted). (`superseded:` is a v2-era spelling kept readable in migrated logs — new entries never write it.) **This is the surface verify's human confirmation renders** — without it, the delta of a run can't be reviewed and confirmation degrades to an unanswerable "정확합니까?". The block's `## YYYY-MM-DD` header is also what bounds "what changed since the human last confirmed", so the date is load-bearing.
...
4. **Propagate + seal.** After the change, run propagation (step 6): any document whose `cited_truths` includes the changed id — or whose `scope_tags` overlap a re-grounded truth — → `status: stale`. Self-audit the change (step 7), regenerate `index.md`/`tree.md` (step 8) and run `weavedoc validate`.
