...
2. **reference** — a truth names an entity that no truth defines. **The noisy kind — bias hard to silence.** Raise only when the undefined entity clearly **carries narrative/structural weight** (a central named character, a role the mine leans on). A background relative, an institution named in passing, or a one-off proper noun is **not** a gap — e.g. `초아 외할머니` or `대형기획사 이사진` are mentioned but incidental → do not raise. When unsure, **don't raise** (note it in the report so the user can overrule).
   - **The axis case — the one sub-kind where the weight rule inverts.** A word the mine uses to *classify* (Phase, 앨범, 시즌, 등급) is not a named entity, so the test above reads it as incidental and drops it — backwards, because an undefined axis silently mis-sorts every card wearing it. Raise when ① the term labels cards across the mine and ② no card states its **criterion** — what decides membership. **Instances are not a definition:** `Phase 1 표기는 …` and `Phase 2 표기가 …` are members of the axis; a mine can hold a dozen of them while nothing says what makes something a Phase. Measured (eclypse): 25 cards carried `Phase` and the criterion card arrived only when the user was finally made to rule on it — until then the machine had supplied its own ("유닛 표기가 같으니 같은 Phase") in prose that no card and no verify lens reads. Evidence is a count of cards using the term against a grep for a criterion-bearing claim, never memory.
