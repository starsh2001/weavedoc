...
2. **Snapshot + convert.** Create `materials/<id>/`; write the raw relevant slice to `source.md` (copy-in, so it stays auditable) and the distilled facts to `converted.md`. Set `origin: conversation` (a gaps/ask answer batch is `user-answer` — same rules below apply). Two hard rules for the excerpt:
   ...
   Then **declare it like any other material** — `intake <id> "<the session and what was declared>"`. The `source.md` snapshot IS the original here, so this is an ordinary digest-bound declaration, not a `--no-source` case; and the note is where "the user declared this stretch as a source" becomes a record instead of a memory.
