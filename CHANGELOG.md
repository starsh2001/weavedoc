...
**Known issues.** ① 읽기 불가 `verify.md`에 대해 plain `status`는 여전히 `human queue: 0`(`--open`은 경고) — status의 출력 계약 변경이라 이번 범위 밖. ② questions.md의 들여쓴 `- [open]`은 컬럼0 항목 문법에 따라 목록에 오르지 않는다(.8에서 이어짐). ③ 중복된 `# Open` 아래에서는 validate가 차단만 하고 세지 않아 목록이 더 많이 보인다(과다 보고, 안전 방향).
