...
**verify 예산 문단의 산술 모순을 정정했다 — 규칙은 무변경, 거짓은 설명이었다.** "예산의 대부분이 재검 라운드에 배정된다"는 문장은 규칙 자신이 계산하는 수와 어긋난다(2026-09-22 평가 실측): 수집은 2–3라운드에 마르고 뭔가 발견한 사이클은 최소 3을 썼으므로, 기본 예산 5에서 재검 몫은 **최대 2** — `standard`(repeat 1)는 여유 1, `full`(repeat 2)은 여유 0이다. 문단을 그 계산으로 교체하고, 예산을 소진시키는 경로가 좁다는 것을 같은 자리에 명시했다: `strength` 바를 넘는 지적이 defender 판정 후에도 **살아남는** 경우(또는 bar-crossing ticket-downgrade)만 2단계로 되돌리며 카운터를 리셋하고, 기각된 지적·바 미달 지적은 카운터를 건드리지 않는다(4단계 clean round 정의 — 이미 있던 규칙의 재서술이다). 평가의 권고 순서 그대로, 기본 라운드 수는 올리지 않았다.

**A1 grounding에 "주장 그대로를 지지하는가"를 구체화했다 — 표본이고, 새 렌즈가 아니다.** 인용이 존재하는 것과 그 인용이 문장을 뒷받침하는 것은 다르다: 원문이 "예외 없이 모든 X"인데 초안이 "Y인 X"로 좁혀도 주제가 언급됐으므로 topic 수준 검사는 통과한다(GroveSpec proof run #5가 코드 쪽에서 실증한 좁힘 클래스 — 주제상 옳은 앵커가 존재 검사를 영원히 만족시킨다). 이제 A1은 수량사·조건·예외를 단 인용 주장 **몇 개를 표본으로** 네 가지를 대조한다: truth 자신의 조건 · plan의 합의 범위(`scope_tags`+섹션 노트) · 초안 표현 · 독자가 가져갈 것. 독자가 다른 집합을 가져가게 좁히거나 넓히면 `unsupported`이고, **plan이 합의했고 초안이 명시한 부분 범위는 위반이 아니다** — 문서는 의도적으로 부분집합을 다룰 수 있고, 결함은 명시되지 않았거나 왜곡하는 좁힘뿐이다. 전수 조사·별도 검토자·반복 조건 추가 없음 — 검사가 스스로 일을 만들지 않게 하자는 같은 검토의 원칙을 이 채택 자체에도 적용했다. 현재 게이트가 이 클래스를 실제로 놓치는지는 커밋 전에 픽스처 광산의 합성 반례 2×2(왜곡 좁힘 초안 vs 합의 범위 명시 초안 × 신규 vs 구형 A1, 콜드 4팔)로 쟀다: 신규 문안은 왜곡을 잡고 정상 부분 범위를 오탐하지 않았으며, **구형 문안도 이 난이도에서는 두 칸 모두 옳게 판정했다** — "현 게이트가 반드시 놓친다"는 도입 시점의 단정은 이 표본에서 기각이고, 보강의 실측된 가치는 능력이 아니라 판단의 명문화다(구형 팔의 옳은 판정은 검토자가 스스로 plan을 참조한 상식에서 나왔다). 표본 1쌍·칸당 1회·초미니 광산이라는 한계와 함께 `notes/FIELD-2026-09-22-a1-synthetic-pair.md`에 기록했다. 더 미묘한 난이도는 다음 완주가 잰다.

검증: `doccheck`(신규 검사 16 포함) 통과 · Windows full regression **647/647 PASS**(-j4) · `git diff --check` · 양 하네스 스킬 사본 3쌍 byte-identical을 `cmp`로 직접 확인 · golden은 `version.txt`의 버전 행만 움직였다. 런타임 변경 없음(bin·schema 무변경, fingerprint `d6bedc204da8` 유지 — 0.7.2와 같은 의도된 실례): 바뀐 것은 스킬 셋의 문안(verify·review·refine, 양 하네스 여섯 파일)·FORMATS·review 템플릿·doccheck이고, 설치본 비교는 manifest가 맡는다. release manifest는 git blob을 해시하므로 스테이징 후 재생성했다: **77 rows, 2회 byte-identical, SHA-256 `a75cd70717ed74fec41fcdfeba5edf69018f96a4e36f54b2ccb9ceb52fa8af87`** (커밋 전 재생성 2회는 0.7.2와 byte-identical(`176cd34a…`)이었다 — index가 아직 이전 번들이었다는 뜻이지 무결성 판정이 아니며, 그 순서 자체를 이 절에 남긴다).
...
**소유자 재정이 전제를 걷어냈다: "이걸 쓰는 사람이 아직 나 빼곤 단 한사람도 없고, eclypse 빼고는 쓰고 있는 광산조차 없어."** 0.6.13이 새 축의 이름을 `authority_level`로 돌아간 이유는 v2 잔재 `authority`와의 충돌 회피였는데, 그 잔재를 지키는 유일한 근거가 "마이그레이션된 광산의 유효성"이었고, 실측상 그런 광산이 없다. 유일한 실광산의 frontmatter에 그 줄은 없고(0.6.13 절의 "기존 광산마다 그 줄이 있다"는 서술 오류였다 — 그 절에서 정정했다), 남은 것은 본문 주석 하나뿐인데 그 주석은 v3가 통째로 지운 "기계적 충돌 해소"를 설명하는, 이 일과 무관하게 이미 틀린 문장이다.

**그래서 시체를 은퇴시키고 낱말을 새 축에 줬다.** ① `project.md`의 `authority`(v2 role-rank 목록): schema optional 목록에서 삭제, `PROJ-AUTHORITY` 검사와 진단 코드 삭제, project.md 템플릿의 `authority: []` 줄 삭제, FORMATS bullet 삭제. ② `conflicts.attribution`(v2 병기 사전 허가 잔재): enum과 검사 삭제 — 병기가 per-entry 사용자 재정이라는 규칙 본문은 그대로다. **어느 쪽도 회귀 케이스가 하나도 없었다** — 그 검사들이 지키던 대상이 없었다는 뜻이기도 하다. ③ 새 축 개명: `authority_level` → `authority`(config 키·plan frontmatter·아홉 스킬의 Authority 서두·인터뷰 문서·doccheck 검사 14·회귀 케이스 셋), 진단 코드 `PLAN-AUTHORITY-LEVEL` → `PLAN-AUTHORITY`. 값과 동작은 0.6.13 그대로다: `strict | standard | delegated`, 부재 = standard, 낯선 값만 차단, upgrade는 아무것도 안 쓴다.

**미발행이라 싼 일이고, 그래서 지금 했다.** 태그 뒤였다면 이 개명은 마이그레이션 항목이 됐을 것이다. 이 번들이 건드리지 않는 한 줄이 남는다: 실광산 project.md의 본문 주석(role rank 설명)은 광산 쪽 파일이라 다음 배포 때 지우는 것이 맞다.

**스키마 변경: 삭제 셋·개명 둘, 마이그레이션 없음** — 지운 키는 어느 광산에도 없고, 개명된 키는 이 브랜치에서 태어나 어디에도 배포된 적이 없다. 650/650.

## 0.6.13

**소유자 재정 셋이 한 대화에서 나왔고, 셋 다 기계와 사용자 사이의 경계를 정한다.** 누가 결정하는가(권한 레벨), 기계가 무엇을 읽는가(조회 규율), 기계가 무엇을 말하는가(보고 규율).

**① 권한 레벨 — 새 config 축 `authority_level`.** 소유자 문면: *"기계와 사용자가 각각 어디까지 권한을 가지고 결정할지, 문서가 어떻게 관리될지 정하는 레벨이 필요해. 문서마다 토씨 하나의 중요성이 다르거든."* 지금까지 config의 축은 전부 검증 쪽이었다 — `verify.strength`(얼마나 막나)·`scale`(검사자 몇 명)·`conflicts.detection`·`fidelity.completeness`. **"누가 결정하나"를 정하는 축은 없었고**, 기계-사용자 경계는 스킬 산문에 하드코딩돼 계약서 광산과 세계관 노트 광산이 같은 경계로 돌았다. `strict | standard | delegated`, 기본 `standard`, **검증 강도와 직교한다**(`strict`+`light`="얕게 검사하되 서명은 전부 사용자", `delegated`+`full`="가장 깊게 검사하되 판정은 기계" 둘 다 성립하는 설정이다). 아홉 스킬이 각자 자기 행을 `## Authority` 절에 싣고, 공통 서두는 아홉 사본 **바이트 동일**이다(doccheck가 대조한다). 문서별 override는 `plan.md` frontmatter의 같은 세 값이고, plan 인터뷰가 묻는다.

**키 이름이 `authority`가 아니었던 이유는 v2 잔재와의 충돌 회피다.** `project.md`의 `authority`(role rank 잔재, legacy-inert)가 규격·validate·FORMATS에 다른 뜻으로 등록돼 있어서 한 낱말이 두 뜻을 갖게 되기 때문이었다. (이 절의 첫 판은 "템플릿이 심어서 기존 광산마다 그 줄이 있다"고 적었는데, 0.6.14에서 실측하니 실광산 frontmatter에 그 줄은 **없었다** — 서술 오류였고, 그 실측이 0.6.14의 개명으로 이어졌다.)

**소유자 교정 둘이 초안 표를 바꿨다.** 보고서의 레벨 표는 기계 초안이었고, 두 지점이 교정됐다. 첫째, 소유자가 *"나는 지금이 strict인 줄 알았어. 기계가 선택지를 제공해서 사용자가 선택한 항목도 나한테 하나하나 다시 다 물어보더라고"*라고 지적했다 — 초안 표가 **작성 권한 축만 그렸고 확인 축을 빠뜨렸다.** 현행 규격은 `adopted` 값을 델타에서 항목별로 강조하고, 일괄 통과한 뒤에도 다음 델타에 다시 나열한다. 그래서 작성 축은 standard인데 확인 축은 strict처럼 동작하고 있었다. 둘째 교정이 그 해법이다: *"사용자가 뭔가를 선택해서 그걸 적용했는데 그게 잘 적용되었는지 확인시켜주려면 그자리에서 말해줘야지."* **전 레벨 공통으로 적용 시점에 결과 문면을 보여 주고, `standard`에서는 그것으로 끝난다** — 사용자가 보이는 교환에서 고른 값은 델타에 목록으로만 실리고 다시 묻지 않으며, 개별 확인은 기계가 단독 판단한 것(계산값·기계가 굳힌 서술·기계가 만든 부정 명제)에만 남는다. 일괄 통과 항목의 재나열은 `strict`에서만 유지된다. 원래 재확인 제도의 목적은 선택의 재론이 아니라 **전사 확인**이었고(기계 제안 158cm가 사용자 값과 구별 불가하게 저장된 사건, 사용자 발언이 기록 중 다른 사실에 잘못 붙은 사건), 맥락이 살아 있는 적용 시점이 그 확인에는 더 강한 자리다. 보호는 유지되고 마찰만 빠진다.

**map의 standard 칸 두 곳은 현행 동작대로 완화해 적었다** — provenance 라벨은 기계가 규칙대로 붙이고(validate가 강제한다), 삭제는 종류로 갈린다(근거 없는 카드의 위생 삭제는 기계+기록, 살아 있는 사실의 폐기는 사용자). 이 완화 덕분에 **"기존 광산 = standard = 현행 그대로"가 성립해서 upgrade 영향이 0이다.** 초안 문면 셋도 정밀화했다: strict gather의 note 금지는 쓰기 대조 줄을 예외로 두고(그 줄은 기계 framing이 아니라 감사 기록이며, 없으면 미감사로 읽힌다), strict map의 "본문 문면 그대로"는 전문 아티팩트가 깨지지 않도록 "본문의 축자 발췌, 재서술 금지"로 적었으며, delegated gather의 사후 승인은 **승인 전까지 자료가 미비준**임을 명시했다(그러지 않으면 v0.6.11이 세운 "승인이 stated 서명을 만든다"가 그 구간에서 정의되지 않는다).

**② 조회 규율 — 감사기록은 문제 분기에서만.** 소유자 문면: *"뭔가를 찾을때 감사기록은 찾지 않고 현재 광산에 있는 데이터들만으로 판정하고, 거기에 문제가 있을때만 감사기록을 뒤져야할것 같아. 안그러면 감사기록이 너무 늘어나서 컨텍스트가 오염돼."* 이 원칙의 절반은 이미 있었다 — search shield가 원문을 내용 검색에서 가리고, READ.md의 소비자 규약이 변환 이력을 시야 밖에 두며, changelog는 어떤 검사 코드도 읽지 않는다. **비어 있던 절반은 작업 중인 에이전트의 시야였다.** READ.md 규칙 1의 audit layer 문장을 넓혀서 `changelog.md`·verify.md의 라운드 이력·두 장부를 같은 층으로 이름 붙이고, 이 조항이 파이프라인 실행에도 걸린다고 명시했다. 경계는 "누가 읽느냐"가 아니라 **"무엇이 나오느냐"**로 그었다: `validate`·`scope`·`census`·`status --open`은 장부를 전부 읽지만 산문이 아니라 계수와 이름을 돌려주므로 그대로 두고, 금지되는 것은 산문 단위로 장부를 퍼 오는 행위다. 실측 근거는 이 프로젝트 자신이다 — 실광산의 changelog는 보고서가 L1605를 인용할 만큼 길다. v0.6.12의 판정자 닫기 규칙(T1)은 이 규율과 어긋나지 않는다: 지적이 제기된 뒤의 문제 분기이고, 요구하는 것이 파일 전체가 아니라 `grep` 한 줄과 그 계수다.

**③ 보고 규율 — 내용으로 말하고 번호는 뺀다.** 소유자 문면: *"자꾸 특정단계에서 m12가 이렇고 t03이 이렇다 하는데 나는 그 내용을 알수 없거든. 일일이 들어가봐야 알지."* 규격이 **정반대로** 적혀 있었다 — 여덟 스킬이 공유하는 "Surface, don't point" 조항이 열린 항목을 "무엇인지(id가 있으면 id)"로 적었고, verify의 확인 단계 1번은 한 줄의 순서를 "id · provenance · claim"으로 못박았으며, 미검증 보고 예시조차 `미검증 1 (m016)` 모양이었다. 이제 사용자에게 가는 문장에서 신원은 **언제나 내용**이고(항목이 많으면 압축하는 것은 설명의 길이이지 설명의 존재가 아니다), 지목이 필요하면 광산 handle을 흘리는 대신 그 메시지 안에서 번호를 매긴다. **기계 층은 불변이다** — frontmatter·coverage·changelog·장부·문서의 인용 주석은 전부 id로 돈다. 마침 그 경계에 이미 번역 단계가 있어서(`status --open` 출력을 붙여넣지 말고 답변 언어로 다시 렌더링하라는 규칙), id를 내용으로 푸는 일이 그 자리에서 일어난다.

**런타임 변경 — 이번에는 바이트가 움직인다.** schema에 `config.enum.authority_level`·`config.toplevel`·`plan.fm.optional` 세 줄, validate에 검사 둘(config는 `CFG-ENUM`, 문서 override는 신설 `PLAN-AUTHORITY-LEVEL`), init 인터뷰에 문항 하나(5→6, 배열은 여전히 2 — 캡이 4라 3+3으로 들어간다). **낯선 값만 막고 부재는 허용한다**: 모든 기존 광산이 이 축보다 앞서므로 필수 키로 만들면 전부를 한꺼번에 빨갛게 만든다(`MAT-UNDECLARED`·`CLAUDE-BLOCK-STALE`이 이미 든 판단). 반대로 낯선 값을 차단하는 것은 같은 논리의 다른 쪽이다 — 인식 못 하는 레벨은 knob을 안 쓴 광산이 아니라, 아무것도 정의하지 않은 낱말을 아홉 스킬이 각자 해석하는 광산이다. `upgrade`는 아무것도 안 쓴다: 쓰인 값과 없는 값은 **누가 골랐는가**에서 갈리고, 그 구별이 이 축에서 유일하게 중요한 것이다.

**회귀 6건 신설.** validate 3건(낯선 config 값 차단 · 부재는 초록·침묵이고 선언은 초록·무경고 — 후자는 `config.toplevel` 누락이 만드는 "enum은 통과시키면서 같은 실행이 knob이 아니라고 말하는" 상태를 잡는다 · 문서 override의 낯선 값 차단), doccheck 검사 14(아홉 Authority 서두 **바이트 대조** + 세 레벨 낱말이 schema의 것과 같은지 + 두 템플릿 + id 규율·조회 규율 각 한 핀). 인터뷰 케이스 둘은 문항 수와 header→schema 매핑을 따라 갱신했다. 바이트 대조가 실제로 재는지는 한 사본을 어긋나게 해 확인했다.

## 0.6.12

**소유자 발안(2026-08-28)이 verify 루프의 순서를 재설계했다: "일단 문제를 몇 라운드에 걸쳐서 최대한 다 긁어모으고, … 적절한 등급 산정을 하고, 그다음 수정 라운드."** 현행 루프는 라운드마다 발견과 수정을 교대했고, 같은 날의 실측 사이클(4라운드·차단 19건)이 그 구조의 비용을 끝까지 쟀다 — **차단 19건 중 11건이 수리가 만든 것**이었고, 원본 결함의 발견은 사실상 R1에서 끝났으며, 발견 쪽에는 관문(연속 클린)이 있는 반면 결함이 실제로 태어나는 수정 쪽에는 아무 기준이 없었다. 연속 클린 카운터는 자기 전제도 깨고 있었다: "같은 것에 같은 질문"을 전제하는데 step 0이 고정하는 것은 `source.*`뿐이라, §7이 지시하는 라운드 간 수리가 `converted.md`를 매 라운드 움직였다.

**§One round가 §One cycle이 됐다: 수집 → 정리·등급 → 수정 → 수정 재검.** 수집은 수정 없이 같은 바이트에 콜드 라운드를 돌리고(새 발견 0이 연속 2회면 마름 — 실측상 2~3라운드; PARTIAL을 낸 라운드는 마름 짝으로 안 센다 — 안 보인 검사는 보인 부재가 아니다), 라운드 사이에는 발견 목록을 do-not-raise가 아니라 **"이 목록에 없는 것을 찾아라"**로 넘긴다 — 다른 렌즈의 재도달은 중복이 아니라 실재의 방증이다(실측 R4 A). 정리·등급은 방어자가 전체 목록을 한 번에: 라운드별 판정이 구조적으로 못 보는 **부류**가 전체 목록에서는 처음부터 보인다(실측: 한 부류가 네 라운드에 걸쳐 한 건씩 나와 매번 개별 수리를 받았고, 이름은 사후에야 붙었다). 수정은 부류당 한 번, 재검은 **전체 읽기**다 — 부류 목록은 "여기를 더 깊이"지 "여기만 읽어라"가 아니다(실측 R4 B: 지적이 가리킨 자리 **옆** 반 문장에 결함이 남았고, diff 한정 검사는 정확히 그것을 못 본다). validate 실패 즉시 수정 예외는 유지된다 — 봉인이 깨진 채로는 수집 자체가 성립하지 않는다.

**`repeat`·`max_rounds`는 config 무변경, 의미만 옮겼다.** `repeat`는 수정 재검 단계의 관문이 됐다 — 고정된 바이트에 연속 `repeat`회, 카운터의 원래 전제가 구조로 복원된다(재검 라운드 사이에는 아무도 대상을 안 고친다). 발견이 나오면 정리·등급으로 되돌아가고 카운터는 0, bar-crossing 리셋 규칙도 그대로 재검 단계에 산다. `max_rounds`는 사이클 전체 예산이 됐다 — 수집이 2~3에 마르므로 예산의 대부분이 재검으로 간다: **검증 강도가 결함이 태어나는 자리로 간다.** 아무것도 못 찾은 사이클은 마른 수집 라운드 자체가 움직이지 않은 바이트 위의 클린 라운드라 `repeat`로 센다. 서술자도 같이 옮겼다(FORMATS의 verify.md 계약과 config 절이 재배치를 명시한다). **옛 §One round대로 기록된 라운드는 유효한 역사다** — 장부(verify-ledger.tsv·verify.md)는 형식이 한 글자도 안 바뀌었고 attest·scope·validate는 그대로 돈다; 새 구조는 다음 사이클부터다.

**수정 규약 4조.** ① 빼기 우선 — 이번 사이클에서 살아남은 수리는 전부 치환·삭제였고 다음 라운드 지적을 낳은 수리는 전부 추가였다; 더하는 수리는 치환으로 안 되는 이유를 함께 댄다. ② 문장 전체 재독 — 반만 고친 문장이 두 라운드 뒤 critical로 돌아왔다. ③ 수정 후 파일 전체 재대조, 산문 포함 — backtick 안만 훑던 대조가 산문에 들어온 세계 사실 진술을 통과시켰다. ④ 냉담 수리자 우선 — 생산자는 자기 수리를 변호하는 해명 산문을 쓰고(그 산문이 이번 사이클 결함의 서식지였다), 방어자가 지적마다 쓰는 수리 방향 한 줄이 지시서가 된다. **④는 미실측 옵션임을 규격이 스스로 명시한다** — 냉담 수리자로 돈 사이클은 아직 없고, 맥락 부족으로 지적을 오독하는 반대 비용이 실재한다.

**판정자 권한 셋 — 전부 권한 이양이 아니라 증거 기준이다(triage-ownership 보고서).** 실측된 비대칭: 같은 라운드에서 같은 파일의 같은 줄을, 검사자가 "광산에 없다"고 주장하면 세 파일 실측으로 반증되는데, 판정자가 "이미 있다"로 닫으려면 실측을 항목 본문에 적어 놓고도 못 닫았다 — 부재 주장은 측정을 요구하면서 존재 주장은 측정을 안 받는 구조가, 광산이 이미 답을 든 항목을 소유자의 결정 목록에 올렸다("근데 이런것까지 사람이 일일이 확인해야하는게 맞아?"). ① 금지 사유 셋 중 **"다른 파일에 기록돼 있음"만은 `grep -F`로 참·거짓이 갈리는 명제다** — 축자 인용 + 파일·줄 + 찾는 명령과 그 계수, 셋을 다 제시하면 `[ruled] [machine]`으로 닫을 수 있다. 부분 제시는 이 규칙이 겨눈 기억 주장 그 자체라 지금처럼 올라간다. `[ruled]`는 `status --open`이 이미 아는 상태 어휘라 런타임 무변경이다. ② **재량 없는 규격 규칙이 do-not-raise 범주를 이긴다** — 판례(adjudications)가 규격을 한 범주씩 조용히 좁히는 경로를 닫는다(실측: 거울 규칙이 강제로 요구하는 값이 note 압축 범주와 겹친다는 이유로 상신됐다; 소유자 재정 후 재측정에서 그 값은 정확히 재현됐다). ③ **소유권 표는 소유자가 되돌릴 수 있다, 항목으로도 범주로도** — "이건 왜 나한테 오냐"는 항목 내용이 아니라 표에 대한 판정이다: `[ruled]`로 닫고 추천안을 적용하되 소유자 문면을 축자로 남기고 기계가 그렇게 읽었음을 명시하며, 같은 종류가 반복되면 do-not-raise 범주로 굳힌다. 지금까지 표를 내리는 길은 항목별 소유자 재정뿐이라("Only a user ruling moves an entry down") 보수적으로 붙은 표가 쌓이기만 했다.

**일부러 안 한 것.** review(advisory) 루프는 그대로다 — 이 재설계는 verify의 실측에서 나왔고, 자기 실측 없는 루프를 따라 바꾸지 않는다(reviewers.md의 Round hand-off에 붙인 수집 단계 예외도 verify 사이클로 한정해 명시했다). 사이클 초안에 있던 `authority: strict` 수리 승인 문장은 뺐다 — 그 config 축은 아직 없고, 권한 레벨 작업이 실릴 때 함께 온다. 옛 재검 등급 표(diff-only mini-round)는 은퇴했다 — 전체 읽기 재검이 그 자리를 대체하며, 실측이 diff 한정 검사가 구조적으로 못 보는 결함을 보였다.

**회귀: doccheck 검사 13 신설.** 사이클의 척추 문장 열하나(수집 무수정 · 마름 조건 · 전체 목록 정리 · 부류당 수정 · `repeat` 재배치 · 전체 읽기 재검 · 빼기 우선 · 미실측 명시 · 표 되돌리기 포함)와 reviewers.md의 판정자 문장 넷(닫기 3요건 · 규격 우선 · found-list 예외), FORMATS의 재배치 문구를 텍스트로 고정한다. v0.6.11 바이트로 되돌리면 정확히 빨개지는 것을 확인했다.

**스키마 변경 없음, 명령 변경 없음, 마이그레이션 없음.** 스킬·FORMATS 문서만이라 런타임 바이트는 v0.6.11과 동일하다(fingerprint 구별 불가 — 번들 라벨로 비교). 기존 설치본은 `.weavedoc/`와 `.claude/skills/weavedoc-*`를 다시 복사하면 된다.

## 0.6.11

**소유자 재정(2026-08-28)이 대화 자료의 기초 동작을 교체했다: "내가 뭔가를 말할때마다 그걸 새로운 원본삼아 gather 하려고 하지마."** 발화가 도착할 때마다 등재하지 않는다 — 대화가 쌓이고, 명시적인 gather 요청이 왔을 때만 움직인다. 그리고 원본은 대화 원문 스냅샷이 아니라 **최종 결론만 담아 제시하고 사용자가 승인한 정리본**이다: 초안에서 기계 제안 값은 `[기계 제안값]`으로 표시해 사용자가 알고 서명하게 하고, 승인된 텍스트가 — 비준으로 이제 사용자의 말이 된 채로 — 바이트 그대로 `source.md`가 되며, 승인 발화는 intake note에 기록된다(승인이 기억이 아니라 기록이 되는 자리).

**원문-스냅샷이 note 층을 강제하고 있었다는 실측이 근거다.** 대화 발화는 구조적으로 지시적이다(`응`·`맞아`·`넷 다 그래` — 선행사가 전부 기계 턴 쪽). 원문이 원본인 한 기계 턴을 `> [machine-note]`로 실어 날라야 하고(v0.6.9의 G1 — 그 전제 아래서는 옳은 의무고, 그 전제의 자료에는 여전히 옳다), 그 층은 설계상 거울 검사 면제 지대다. 실광산(2026-08-28): 자료 넷에서 note 층이 본문의 7~26배로 자랐고, 4라운드 콜드 사이클의 차단 지적 전부가 기계가 쓴 산문 — note 층과 frame(title·summary·소제목) — 에서 났다. 본문은 한 번도 안 틀렸다. 정리본은 자기완결 서술문이라 나를 선행사가 없고, note 층의 존재 이유가 사라진다.

**provenance가 정리된다.** 비준이 전 문장을 `stated`로 만들고, 표시된 채 승인된 기계 제안 값만 `adopted`가 된다 — 정리본 원본 안의 표시가 곧 가시적 채택 교환이다. 규칙의 소유자 셋을 같이 가르쳤다(한쪽 소비자만 가르친 규칙은 이 저장소가 반복해서 지워 온 결함 클래스다): **gather**가 표시를 쓰고, **map**이 body의 표시를 `adopted`로 읽으며(표시는 주석이지 content가 아니라 claim에는 절대 안 들어간다 — 표기 관례 carve-out과 같은 종류), **FORMATS**가 converted.md body 계약과 `adopted` 정의에 그 표시를 싣는다.

**범위는 좁다.** ask-loop 배치(`origin: user-answer`)는 다른 모양이라 — 질문이 기계 것이고, 답이 그것에 기대며, 등재가 사용자가 이미 돌리는 루프의 일부다 — 종전대로 교환 원문을 스냅샷하고 세 규칙(settling exchange 보존 · 질문→서술 승격 금지 · 기계 턴 carry)을 그대로 따른다. 그리고 **소급하지 않는다**: 원문-교환 모양으로 등재된 기존 자료는 그대로 두고 그 모양을 만든 규칙 아래서 계속 읽는다 — 새 절은 새 등재만 다스린다.

**회귀: doccheck 검사 12 신설.** `[기계 제안값]` 토큰이 세 소유자(gather·map·FORMATS)에 다 있는지, gather가 규칙의 척추 문장 일곱을 그대로 싣는지, map·FORMATS가 각자의 소비자 문장을 잃지 않았는지를 텍스트로 고정한다. v0.6.10 바이트로 되돌리면 정확히 빨개지는 것을 확인했다.

**스키마 변경 없음, 명령 변경 없음, 마이그레이션 없음.** 스킬·FORMATS 문서만이라 런타임 바이트는 v0.6.10과 동일하다(v0.6.3처럼 fingerprint로는 구별 불가 — 번들 라벨로 비교). 기존 설치본은 `.weavedoc/`와 `.claude/skills/weavedoc-*`를 다시 복사하면 된다.

## 0.6.10

**콜드 리뷰가 v0.6.9의 훅 게이트에서 셋, 세는-말 잔재 하나를 짚었다.** 넷 다 작고, 셋은 실패 방향이 안전했다 — 그래서 릴리스를 되돌리지 않고 이 번들로 닫는다. 리뷰는 15개 넘는 공격 축을 실측으로 기각했다(CRLF frontmatter · quarantine·headless 의미론 · `..` traversal · 8.3 단축명 우회 · TTL 경계 · lease 경쟁 등 전부 설계대로 동작).

**게이트가 config의 `paths:`를 몰랐다 — 재배치된 광산에서 아무것도 지키지 않으면서 침묵했다.** 규칙 표가 `materials/`·`truths/`·`documents/`를 **리터럴**로 들고 있었는데 이 셋은 옮길 수 있는 값이다. 측정: `materials: data/materials`인 광산에서 리스 없이 `data/materials/…`를 쓰면 **허용**됐고, 존재하지도 않는 루트 `materials/`만 거부했다. 게이트가 빈 이름을 지키고 진짜 트리를 통과시킨 것이다 — 이 파일이 스스로 이름 붙인 "아무것도 재지 않으면서 성공을 보고한다" 클래스인데, 게이트의 실패 방향이 allow라 **조용했다**. 해소는 런타임의 단일 `paths:` 리더(`cfgPath`)를 그대로 import하는 것이다 — 게이트와 validate가 한 광산의 materials를 두 곳으로 해석할 수 없게 된다. 매칭은 root-relative 접두 대신 **절대 포함**으로 바꿨다: `paths:` 값은 광산 밖을 가리켜도 합법이고 rel 문자열로는 그것을 표현할 수 없다. 리터럴 규칙을 먼저 보므로, 폴더를 `.weavedoc/` 안으로 돌린 광산은 여전히 번들 거부를 만난다.

**`HOOKS-STALE`가 따옴표 친 명령에서 영구 오탐이었다.** 마커부터 끝까지 자르는 tail에 **닫는 따옴표**가 들어갔다: `node "C:/my mine/.weavedoc/bin/hooks/gate.mjs"` — 경로에 공백이 있으면 이것이 유일하게 동작하는 철자인데, 그 하나가 영영 stale로 읽혔다. 블록이 선언한 "접두는 환경이고, 거기 발화하면 늑대소년이 된다"와 정면으로 어긋난다. 마커 시점에 따옴표가 **열려 있으면** 그 닫는 짝을 tail에서 뺀다. 스크립트 뒤의 인자는 여전히 센다 — 그건 환경이 아니라 무엇이 도는지를 바꾼다.

**메시지 구분자가 인코딩 경계를 넘지 않았다.** `have.join(' · ')`의 가운뎃점이 `M`의 보간 값으로 그대로 실려 latin1 경로에서 홀로 선 `0xB7`로 나갔다 — 같은 문장의 리터럴 절반은 정상 UTF-8이라 **한 메시지에 두 인코딩**이었다. 이 파일이 자기 리더 경계에 이미 적어 둔 클래스다. `U(' · ')`.

**그리고 `weavedoc-init`의 세는 말 하나가 남아 있었다** — 가드를 넷으로 늘리면서 그 줄의 꼬리 "(Both bullets are marked …)"를 안 고쳤다. v0.6.9가 걷어낸 바로 그 표류가 내가 편집한 그 줄에 남은 셈이다.

**회귀 2건 신설 · 647/647.** 둘 다 **v0.6.9 바이트로 되돌리면 정확히 빨개지는 것**을 확인했다 — 결함을 잠그는 테스트가 아니라 검출하는 테스트다. `acct_hook_gate_follows_config_paths`는 재배치된 트리의 거부와 **비워진 리터럴 이름의 통과**를 함께 잰다(둘 다 재지 않으면 "이름 두 개를 다 지킨다"는 오답이 통과한다). `pass_hooks_quoted_command_is_not_stale`은 공백 든 절대 경로의 따옴표 형태가 침묵하는지 잰다.

**스키마 변경 없음, 명령 변경 없음, 마이그레이션 없음.** 기존 설치본은 `.weavedoc/`와 `.claude/skills/weavedoc-*`를 다시 복사하면 된다.

## 0.6.9

**실광산 자료 축 콜드 검증 라운드가 차단 7건을 냈고, 일곱 전부가 "이 문장은 누구 것인가" 실패였다.** 값이 틀린 것은 하나도 없었다. `converted.md`가 기계 턴 다섯을 전부 떨어뜨린 채 그 자료의 `summary`가 "2집 유나"를 단언했는데, 그 값의 유일한 선행사가 떨어진 기계 턴 안에 있었다 — search shield가 `source.md`를 규약 준수 독자에게 도달 불가로 만드니, 값은 어디서도 접지되지 않는다. 카드 두 장이 `provenance: stated`를 입고 본문에 **0회** 등장하는 문구(`(유닛 표기)`·`2집은 유나 서사`)를 claim에 실었다. 파생이라는 자백은 같은 파일 machine-note에 정직하게 적혀 있었고, 한 홉 뒤에 세탁됐다. note 하나가 `무관한 카드 21건(실측)`이라 적었는데 어느 스코프도 21을 카드로 재현하지 못한다(`pull` 5 · 카드 grep 15 · 21은 원장 넷을 포함한 **파일** 수). 자매 자료는 "8종"이라 쓰고 아홉을 나열했다.

**뿌리는 둘이다.** 하나, **진실 카드에는 문장 단위 provenance 규율이 있고 자료에는 없다** — 그런데 `title`·`summary`·`machine-note`는 기계가 쓴 산문이고 `catalog.md`와 조회 표면에 그대로 실린다. 둘, 그 작업 전부가 **소유 스킬을 로드하지 않은 세션**에서 나왔다: 한 verify 워크스루가 자료 8건·카드 23장을 인라인으로 생산하는 동안 gather/map은 한 번도 로드되지 않았고, 차단 7건이 전부 그 미로드 작업에 몰렸다. 로드된 스킬 자신의 절차는 끝까지 깨끗했다. 컨텍스트 양이 아니라 **규칙 거리**가 부식시킨다.

**규칙 넷이 기본 동작이 됐다(스킬 본문 = 설치한 모든 광산에 배포).** ① gather: 사용자 답이 기계 턴에 기대면 그 턴을 `> [machine-note]`로 `converted.md`까지 실어 나른다(새 note 종류는 만들지 않았다 — 아래) · 실측 수치는 **측정일과 명령**을 지닌다, 재실행할 수 없는 숫자는 쓰지 않는다 · 쓰기 대조 결과가 자료 안에 마지막 줄로 남는다(`쓰기 대조 <date> — …`). ② map: **claim 내용은 본문에서만 온다** — `title`·`summary`·note는 기계 산문이지 추출원이 아니며, 기계 턴에만 있는 값은 `adopted`가 되거나 카드가 되지 않는다(표기 관례는 형식이지 내용이 아니라는 예외를 명시). ③ 아홉 스킬 전부가 **바이트 동일한** 스킬-핸드오프 블록을 싣는다 — "The work's owner is the skill, not the session". ④ init이 다섯째 가드를 심는다: `.claude/settings.json`의 훅 두 개.

**훅 게이트 — 규칙 로드를 기억에 맡기지 않는 유일한 층.** 리스 훅(`PostToolUse`·`Skill`)이 세션이 어느 weavedoc 스킬 아래 있는지 기록하고, 게이트 훅(`PreToolUse`·`Write|Edit`)이 소유 스킬 리스 없는 광산 쓰기를 거부한다. **거부 메시지가 곧 규칙이 로드되는 순간**이라서, 그 문장이 산출물이다. 경로→소유 스킬 표는 새로 정한 것이 아니라 아홉 스킬의 `Write-scope` 선언을 화해시킨 것이다(그 과정에서 모순 셋을 고쳤다: gather가 `questions.md`와 `.ignore`를, refine이 `catalog.md`를, verify가 `coverage.md`·`changelog.md`를 쓴다고 선언에 없었다). **한계를 규격에 명시했다** — 게이트는 "소유 스킬 없이 광산을 쓴다"까지만 보고 "어느 스킬 소유인가"는 못 본다, 셸 경유 쓰기는 통과한다, 리스는 위조 가능하다. 방어 대상은 **망각**이지 악의가 아니고, 내부 오류에는 **fail-open**한다(모든 광산 쓰기를 막는 게이트는 하루 만에 꺼지고, 그건 안 만든 것과 같은 결말에 신뢰만 잃는다).

**런타임 검사 하나 — `MAT-STATUS-LEDGER`(차단).** 실광산에서 자료 둘이 원장 승리 행 `verified` 아래 frontmatter `converted`인 채 **3주 가까이** validate 초록이었다. `attest`는 원장만 쓰고 frontmatter를 건드리지 않으니, 스킬의 나머지 절반을 건너뛰면 도구가 남기는 상태가 정확히 그것이다. **축은 여전히 분리돼 있다**(WD-COR-001): `used`·`retracted`는 나중 lifecycle이라 합법, `legacy-unbound`는 아무것도 판정하지 않고, 행 없는 frontmatter `verified`는 보존된 v1 역사다. **digest 비교는 하지 않는다** — 바이트가 아직 맞는지는 `scope`의 판정이고, 여기서 digest를 보면 0.6.8이 재고 접었던 둘째 판사가 된다. 원장이 손상됐거나 머리 없는 행이 있으면 **통째로 정지**한다.

**일부러 안 한 것.** 새 note 종류를 만들지 않았다 — `[note]`/`[machine-note]`는 일곱 표면 계약이고 여덟째 동기화 비용을 사지 않는다. 훅 게이트가 스킬 규칙을 대체하지 않는다 — 아홉 사본이 여전히 본체다. 쓰기 대조 줄을 기계 검사로 강제하지 않았다 — 그 줄은 주장이고, 검증은 콜드 레인의 재현이다. 지시어 선행사 검출은 규격이 자리 잡은 뒤로 미뤘다. `catalog.md`의 상태 열은 대조 대상에서 뺐다 — 그 파일은 frontmatter에서 **생성**되는 것이라 거울이 맞으면 재생성으로 따라온다.

**회귀 33건 신설 · 645/645 · doccheck 검사 2개 추가(8b·11).** 구현 중 실제 결함 둘을 이 케이스들이 잡았다. 첫째, Windows가 같은 디렉터리를 8.3 단축명과 장문형 두 가지로 내주는데 `realpathSync`는 받은 표기를 보존하고 `.native`만 정규화한다 — 훅이 단축명 경로로 실행되고 대상이 장문형으로 지목되면 포함 검사가 false가 되어 **게이트가 모든 광산 쓰기를 조용히 허용하면서 exit 0을 찍고 있었다**. 둘째, 튜플 구분자로 들어간 리터럴 NUL 바이트 세 개가 grep에 모듈을 바이너리로 보이게 만들어 **진단 코드 표 검사가 파일을 중간에서 읽다 말았고**, 실제로 방출되는 코드 27종을 "코드에 없다"고 보고했다. 둘 다 "초록을 찍으면서 아무것도 재지 않는" 클래스이고, 그래서 이 저장소가 두 검사를 들고 있다.

**스키마 변경 없음, 명령 변경 없음, 마이그레이션 없음.** 번들에 파일 셋이 늘었다(`bin/hooks/lease.mjs` · `bin/hooks/gate.mjs` · `templates/hooks.json`). 기존 설치본은 `.weavedoc/`와 `.claude/skills/weavedoc-*`를 다시 복사하고, 게이트를 원하면 `weavedoc-init`을 reconfigure로 한 번 돌리면 된다.

## 0.6.8

**`impact`의 모든 절이 앞방향이었다.** 자료 → 카드 → 문서만 따라갔고 "이 자료를 고친 자료"는 어느 절에도 없었다. `corrects:`는 **정정하는 쪽**의 frontmatter에 선언되기 때문에, 정작 경고가 필요한 독자 — 정정당한 자료를 여는 사람 — 이 그것을 볼 수 없는 유일한 사람이었다. 실광산 측정(eclypse): `m003`을 고치는 자료가 **6건**(m034·m035·m036·m038·m039·m041)인데 `impact m003`은 그중 하나도 이름을 부르지 않았고, 정정에 관한 한 아무도 손대지 않은 자료와 **구별되지 않았다.**

**원인은 그 필드의 독자가 한쪽뿐이었다는 것이다.** `corrects:`를 읽는 코드는 validate의 존재 검사(`MAT-CORRECTS-DANGLING`·`MAT-CORRECTS-SELF`)뿐이었다. 쓰는 쪽은 검사되는데 되읽는 쪽이 없었다 — 장부는 있고 조회가 없는 상태다.

**바뀐 것 셋.** ① `impact`가 **역방향 절을 맨 앞에** 출력한다. 아래 나오는 카드 일부가 이미 대체됐을 수 있으니 그것은 나머지 전체에 걸리는 단서지 부록이 아니다. 정정 항목은 `m003 §4.4`처럼 **어느 절을 대체하는지까지** 그대로 싣는다. ② 그 필드의 해석이 core의 `correctsRefs` **하나로 모였다** — validate가 앞방향으로, `impact`가 역방향으로 같은 리더를 쓴다. 한 규칙을 한쪽 소비자에게만 가르치는 것은 이 런타임이 반복해서 지워 온 결함 클래스("two spellings of one question")고, validate의 지역 헬퍼를 복사했다면 그 자리에서 재현됐다. ③ `(none)`을 **출력한다** — 침묵은 "정정이 없다"와 "이 판본은 보지 않는다"를 구별하지 못하고, 비어 있음이 주장이 되지 않으면 이 절은 값이 없다.

**저장하지 않는다.** 역링크는 매 실행 자료에서 훑고 파일로 남기지 않는다. 저장한 역링크는 frontmatter가 이미 들고 있는 사실의 두 번째 사본이고, 진실 층이 바로 그 모양(graph.md edges)을 지웠다 — 어긋날 수 있는 기록을 둘 두는 대신에.

**retracted 정정자는 지우지 않고 표시한다.** 회수된 자료는 그 시점부터 아무것도 접지하지 않으므로 그 정정도 서지 않는다. 그렇다고 줄을 빼면 독자는 그 대목이 **한 번도 다투어진 적 없다고** 믿게 된다. 줄은 남기고 철회를 그 줄에 적는다.

**문서 셋.** `FORMATS.md`에 **changelog 귀속 규칙**이 들어갔다 — 판정은 여기에 서술될 수 있어도 여기서 **발생할 수는 없다.** 실측한 뿌리는 로그가 아니라 한 층 아래였다(eclypse 2026-08-26): `c003`이 `소유자 판정: 아이돌 재즈팝`으로 적혔는데 그 장르는 해당 자료의 질문 넷에 **없었고**, 문구의 출처는 **기계가 자기 추천에 붙인 근거**였다. 그 광산이 스스로 만든 방어(`기계가 각 추천에 붙였던 근거 (제안의 일부이며, 사용자 진술이 아님)` 표제)가 나중 라운드에게 위조를 물려받는 대신 **측정하게** 해 줬고, 그 표제를 규칙으로 올렸다. `weavedoc-gaps`에는 `reference` 렌즈의 **axis 하위 규칙**이 붙었다 — 분류에 쓰는 낱말(Phase·앨범·시즌)은 명명된 실체가 아니라서 기존 무게 규칙이 **정확히 반대로** 떨어뜨린다. 실측: `Phase`를 쓰는 카드가 25장인데 **기준을 담은 카드는 사용자가 재정할 때까지 없었고**, 그동안 기계가 자기 기준("유닛 표기가 같으니 같은 Phase")을 어느 카드도 어느 렌즈도 읽지 않는 산문에 적어 두고 있었다. `weavedoc-gather`의 회수 절차는 정정 목록을 **먼저** 읽는다 — 이미 여섯이 고친 자료는 대개 회수가 아니라 교체 사안이다. 그리고 이 번들에는 앞선 작업에서 남아 있던 `weavedoc-map` 라우팅 수정이 함께 실린다 — 충돌 해소(`충돌 해소`·`c002 판정`·`resolve conflict`…)가 map의 트리거 목록에 없어서, 열린 충돌을 처리해 달라는 요청이 그 일을 실제로 수행하는 스킬로 가지 않았다.

**넣으려다 실측으로 접은 것 둘.** 원장 verdict와 frontmatter `status`를 대조하는 검사를 validate에 넣으려 했으나, eclypse 자료 43건 전수 대조에서 "원장 verified · frontmatter 약함"은 **0건**이었고 반대 방향(m019~m022)은 `scope`가 이미 `legacy-unbound`로 **이름까지 불러** 보고하고 있었다. 넣었다면 `scope`가 소유한 판정의 두 번째 판사가 됐을 것이다. changelog의 미확인 블록을 세는 것도 접었다 — 그 파일을 읽지 않는 것은 공백이 아니라 §1.4의 선언이고, 세려면 새 파서가 필요하다.

**회귀 3건 · 612/612.** 역방향이 뜨는 것, `(none)`이 주장인 것, retracted 정정자가 철회 표시와 함께 남는 것을 각각 잰다. 첫 케이스는 역방향이 **앞방향으로 새지 않는 것**도 함께 재고(정정 항목이 "문서가 인용한 것"으로 세어지면 안 된다), 같은 필드를 앞방향으로 읽는 `MAT-CORRECTS-DANGLING`이 여전히 막는 것까지 한 케이스 안에서 확인한다 — 리더를 하나로 합친 수정이라 두 방향을 같이 재지 않으면 절반만 잰 것이 된다.

**스키마 변경 없음, 명령 변경 없음, 마이그레이션 없음.** 읽기 전용 출력에 절이 하나 늘었다. 기존 설치본은 `.weavedoc/`를 다시 복사하면 된다.

## 0.6.7

**`scope`가 legacy-unbound를 보고만 하고 해소 명령을 말하지 않았다.** 실광산에서 나왔다 — `truths/verify-ledger.tsv`에 `legacy-unbound` 행 223건이 이주 한 번으로 생긴 뒤 **석 달 동안 매 실행마다 보고되면서 한 번도 닫는 방법을 듣지 못했다.** 소유자 발화가 결함을 그대로 짚는다: *"레거시 언바운드가 있으면 그게 맞는 자료인지 확인을 해야지 왜 가만히 있는거야?"*

**원인은 게이트 하나였다.** 해소를 안내하는 유일한 문장("re-verify by risk …")이 `owed === 0` 분기 **안에** 있었다. 그런데 작업 중인 광산은 거의 언제나 owed가 0이 아니다. 즉 backlog를 가진 광산이 정확히 그 안내를 못 받는 광산이었고, 대신 출력되는 마무리 줄은 "a round examines the sets above"뿐이라 legacy 축을 아예 언급하지 않는다. golden 스냅샷이 이미 그 상태를 고정하고 있었다(`scope.txt`: t001 legacy-unbound · m001 owed · 안내 없음).

**바뀐 것 둘.** ① 해소 안내가 **게이트에서 나왔다** — legacy 단위가 하나라도 있으면 owed와 무관하게 출력되며, 명령을 문자 그대로 적는다(`weavedoc attest verified <round> <standard> <id...>`)와 함께 "스탬프가 아니라 **재검증**으로 닫힌다 · 위험 순으로, 일괄은 안 된다"를 병기한다. ② intake 축의 legacy 줄이 **경고를 같이 들고 간다** — `anchored ≠ verified`는 지금까지 `anchored` 줄에만 있었는데, 그 줄은 **이미 실행한 뒤에** 뜬다. 경고가 필요한 독자는 아직 실행하지 않은 쪽이다. 이제 `intake --anchor-existing`을 권하는 그 문장이 "디스크에 있는 것을 그대로 **채택**하며 누가 읽었다는 뜻이 아니다 — 트리가 의도한 그것인지 **먼저** 확인하라"를 함께 말한다.

**설계는 뒤집지 않았다.** 이주는 여전히 digest를 자동 발급하지 않고(`FORMATS.md` §`intake --anchor-existing`의 근거 — 이주가 찍은 digest는 나중에 증거로 읽히지만 실은 도구가 돈 순간의 디스크를 적은 것이고, 십 분 전 편집도 그 안에 들어간다), 이미 binding이 있는 자료를 재anchor하지 않으며, `source.*`가 없어 ruling이 필요한 자료는 종전대로 이름만 불리고 건너뛴다. 여기서 새로 쓰이는 바이트는 없다 — **명령을 말하는 것이 명령을 대신 실행하는 것은 아니다.**

**회귀 4건, 음성 확인 완료.** 핵심 케이스는 **owed가 있는 상태에서** 안내가 뜨는 것을 재고(수정 전 실제로 빨갛다), 두 번째가 `owed === 0`에서도 카운트 줄과 안내가 **둘 다** 남는 것을 재며(분기를 맞바꾸는 수정이면 구멍이 반대편으로 옮겨갈 뿐이다), 세 번째가 닫을 것이 없으면 **침묵하는 것**을 잰다(무조건 뜨는 안내는 장식이고, 장식은 다음 진짜 줄이 안 읽히는 경로다). 네 번째는 intake 축 경고가 권유 지점에 붙어 있는지를 `acct_intake_backfill_on_an_already_v3_mine`에서 잰다. golden `scope.txt`가 새 2줄을 고정한다.

**스키마 변경 없음, 명령 변경 없음, 마이그레이션 없음.** 읽기 전용 보고가 두 줄 늘었고 기존 문장 하나에서 한 절이 아래로 옮겨갔다. 기존 설치본은 `.weavedoc/`를 다시 복사하면 된다.
...
**Harness note (no bundle bytes changed).** The tag's first CI attempt was blocked by `meta_key_covers_the_git_index`, a meta-case that spawned a **nested `regress.sh` child** to compute a key for a scratch tree. Seven rounds chased that child's failures around — a silent `git add`, a silent `git init`, then an empty key — always green solo and red only under a loaded `-j6` sweep. The fix stopped relocating the symptom: `compute_key` is now **defined unconditionally** (it lived inside the driver-only branch, which is the sole reason a nested child was needed at all), so the case computes both keys **in-process** with `$REPO` repointed. That deletes a bash startup, a 5,000-line re-parse and ~25 spawns per measurement, and the flake with them. The probe also moved to `WORKFLOW.md` — tracked but **not content-hashed** by the key — so staging it can move the key through exactly one path, the index, and the fragile worktree-restore step is gone entirely. `tests/` is not part of the bundle manifest: fingerprint `06f3106b039d` and the 64-row manifest are byte-identical to what the tag names.
...
**`meta_key_covers_the_git_index` staged its probe with rc unchecked and stderr discarded.** Under a `-j6` sweep on MINGW the `git add` sporadically failed outright, and the case then reported *"the index is not in the key"* — a key defect it had not found, wearing a parallel-only-flake face (solo runs always passed; observed twice, `before==after==`the unstaged key). The probe is now **verified by the index listing** — never rc alone, and never `git diff --cached`, which in this commit-less scratch repo has no HEAD to diff against (the first hardening attempt used it and manufactured a persistent false fixture-failure) — with three bounded retries; on true refusal the case fails as a **fixture fault quoting git's own words**, never as a false key verdict. And the key comparison itself now **measures twice on equality**: the subject is deterministic code, so a key that truly stopped covering the index answers the same on every re-measure and stays red — while the environmental shape this case kept producing under load (staging *verified* in the parent, one child answering with the unstaged key — observed on MINGW `-j6` and once on the Linux leg) is absorbed, with both readings and the parent's own index hash kept in the failure diagnostics so a genuine red names which side disagreed. The scratch build also stopped staging the whole copied tree: the assertion only needs an index whose *listing* moves when the probe lands, so the index is seeded with **one file** — the flake's next relocation was exactly the old `git add -A` (CI went red one step earlier, "could not build a scratch git repo", on two OSes), and every git step in the case now fails loudly with git's own words instead of a verdict about the key.
...
**Why that is worse than a missing check.** The agent that writes a material also writes the cards that cite it. An invented material is therefore *structurally perfect*: its quotes match its source because the same hand wrote both, `coverage.md` closes, the verbatim seal passes, `validate` is green. Every check added downstream of a forgeable root only makes the mine state the forgery with more confidence — `247/247 sealed` means *the cards agree with the materials*, never *the materials agree with the world*. A real run (2026-08-14, the eclypse mine) walked most of the way there: the owner said "delete the Knock:One personal narrative" — an instruction about the **mine**, not a fact about the world — and the session began registering it as correction material `m033` with `corrects: [m003 §6]`. What stopped it was one incidental judgment, *there is no original to quote*. A plausible invented sentence would have passed `validate` intact. And the rule was already written, twice, in `weavedoc-gather/SKILL.md` ("Set aside anything that's a **direction about the document** — that's not a material"; "the user must **declare** it; never silently absorb prior chat as a source"). **The rule existed and nobody re-read it.** That is the shape of the defect, and it is why this bundle does not add a sentence to a skill.

**The fix is not new — it is the design that was only ever applied to one lane.** The verification lane has had this device since v0.3.2: a machine-owned append-only ledger, one row per unit, digest-bound, `legacy-unbound` for what predates it, counted apart and never absorbed. `materials/intake-ledger.tsv` is that device applied to intake. `weavedoc intake [--no-source] <material-id> <note>` writes it — a **tree digest over every `source.*` in the folder** (so adding, removing, renaming *or* editing an original moves it), how many files that digest covers, and a note recording how it arrived. `validate` warns `MAT-UNDECLARED` for a material with no row; `MAT-INTAKE-LEDGER` names ledger damage *before* those lines, because a permissions fault must not read as "every material in this mine was fabricated". `scope` prints the lane split — `declared (digest-bound) · no-source · legacy-unbound · stale · undeclared` — and is where the digests are actually **compared**, since it is the command that re-reads every original; `validate` and `census` report row presence and say so, so neither prints "digest-bound" for a comparison it did not run.

**It does not prevent fabrication, and designing it that way would have been the more dangerous choice.** An agent can call `intake` and invent a `source.md` in the same breath. What is removed is fabrication's status as the **default** and its **invisibility**: a silent file write is no longer sufficient, the alternative leaves a row with a digest bound to bytes that had to exist on disk, and a material with no row is named out loud. The judgment stays with the human, where this tool puts every judgment.

**A WARNING, never a block — and that is a constraint, not a preference.** Every mine in existence predates this ledger, so a blocking version would redden every project at once and be switched off within a day: the same outcome as not shipping it, minus the trust. (The judgment `.27` shipped one bundle earlier under `CLAUDE-BLOCK-STALE`: *a warning, never a problem, and it must not block a ship.*) `upgrade --apply` backfills a pre-ledger mine's rows as `legacy-unbound` — real history that binds no bytes — on **both** paths: inside the v2→v3 migration, and on an already-v3 mine, which is why `upgrade` no longer returns "already schema v3" from the version field alone but checks whether that backfill is still owed. After the backfill, `MAT-UNDECLARED` means what it is supposed to mean: a material that appeared *after* the ledger existed and was never declared.

**`--no-source` is the one ruled exception, and it is load-bearing.** Without it the check dies of a single true case — the real mine's `m028`, where the user ruled "원본 없음 — 대조할 대상이 없으므로 검사 자체가 해당 없음". One permanent false alarm is how a warning stops being read. It is not free: the ruling goes in the note, it binds no bytes, and it is counted apart in `scope`, `census` and `FORMATS.md`.

**Three subtractions, not additions.** The append-only ledger *transaction* (atomic create, torn-row refusal, verified rollback, and the created-here case where "as before" means absent) moved to `write.mjs` and now has one spelling for both ledgers — `attest`'s sentences are byte-identical, its fault-injection harness drives the shared body. The ledger *reader* is `verify.mjs`'s, parameterised over the row filter and the recognised words rather than copied. And `raw-source-model.mjs` — shipped hardened in Phase 1 and marked "UNWIRED, no production consumer" — is now wired: `intake` is its first consumer, which makes its refusals load bearing (a `source.*` that is a symlink, a junction, a hardlink to `converted.md`, or a file rewritten mid-read cannot be sealed as an original).

**Two cross-column clauses carry the whole file.** A word that binds a source (`declared`, `anchored`) must hold a 64-hex digest *and* a positive source count; a word that binds a copy (those two plus `no-source`) must hold a 64-hex `copy` digest. `intake` always computes what it writes, so a hand-written row carrying `-` claims a binding to bytes nobody hashed — this ledger's own subject, performed on the ledger. Such a row is not a declaration with a missing field; it is damage, and the material reads as undeclared. **Checks:** 28 regression cases — declared·undeclared·the `--json` warning surface·the no-source refusal and its ruling·staleness on each side separately·a tab in the note·both hand-written binding-without-digest rows·an unknown declaration word·a headless row·an unreadable ledger·a ghost row·a retracted material·an aliased source·the census split on both of census's exits·the backfill on an already-v3 mine and inside the v2→v3 migration (with its report)·the anchor batch, its anti-laundering refusal, its partial-batch naming, and the lifecycle stamp *not* reading as a copy edit. The dirty-worktree asymmetry is deliberate: the v2→v3 path is a transform whose only undo is git, while the backfill appends to one machine-owned ledger and touches nothing else — the same class as `attest`, which requires no backup either. Demanding one would have put the friction on the single command every existing mine must run to adopt the ledger, at the moment its worktree is dirty from ordinary work. The pristine fixture now carries an original and declares it **by running the command**, so every sweep exercises the write path and the states this ledger distinguishes are testable apart.

**The copy is bound too — the half the source digest cannot see, added after the incident finished playing out.** The tree digest catches an *original* being rewritten and says nothing about `converted.md`. The same real run (2026-08-14) showed that is the half that gets edited: the owner's "drop it from the project" was carried out by **deleting a column out of `materials/m003/converted.md`** — the mine's own record of what the source document said — and the truth cards quoting that table were re-cut to match, so seals, coverage and `validate` stayed green (`247/247 sealed`) while the record disagreed with its original. The material had never been verified, so the verify lane's digest did not exist yet; nothing on the mine could name the edit. Now every intake row also carries a **`copy` digest of `converted.md`** — through the verify lane's own `matDigest` (frontmatter `status:` excluded, so a lifecycle stamp does not cry stale), never a second spelling of "the material's bytes". `scope` reports the two sides apart — `stale (source)` and `stale (copy)` — because they fail for different reasons and want different answers, and the copy line names the one distinction the machine cannot judge: *a conversion FIX belongs here; a DECISION about the mine does not — retract the truths or enter a correction material, and never edit the copy to say what the original did not.* Reproduced on a copy of the real mine: `validate` green, `scope` names `m003`.

**`intake --anchor-existing <note>` — the migration answer, as a person's act.** A pre-ledger mine's backlog binds nothing and just sits there, editable on either side without trace. The way out cannot be for `upgrade` to hash the backlog on its way past: a digest minted by a migration reads afterwards as evidence while recording whatever was on disk the moment a tool ran — *including an edit made ten minutes earlier* — which is how a falsified copy becomes canon, and the real mine came within one command of exactly that. So binding the backlog is a separate, explicit act by a person vouching for the tree. It writes **`anchored`, a fourth word, on purpose**: collapsing it into `declared` would let a mine that anchored its backlog this morning report "32 declared" — overstating the record, the one failure this ledger exists to prevent, committed about itself. Anchored is **not** verified (the command says so in its own output), it never re-anchors a material that already carries a binding — stale or not, because adopting whatever a stale material was edited into would turn the command that exists to *expose* an edit into the one that buries it — and materials needing a ruling (no `source.*`) are named and skipped, never batched. `upgrade` now **reports** the unbound count and what it costs instead of minting its rows in silence: leaving a backlog unbound was always a decision, and it was being taken on the owner's behalf without ever being put in front of them.

**One extraction, and a guard it forced.** The append transaction's eight outcomes are spelled once (`sayAppendFailure`) instead of twice, and both consumers' outcome tables now refuse an unhandled kind with a sentence naming it rather than printing `undefined` — the day `write.mjs` grows a ninth outcome and only one consumer is taught it, the failure says so instead of saying nothing.

## 2026-08-08.27

**Unreleased — the pointer that outlived the schema it described.** A field report on a migrated mine (eclypse, bundle `.26`, `validate` PASS) found that the block `init` plants in the project's `CLAUDE.md` was still the v2 one. Nothing had gone wrong with the migration: `upgrade --apply` moved the schema, the validator, `READ.md` and 271 cards to v3 exactly as designed — and then the one artifact sitting *outside* the mine stayed behind, wearing `<!-- weavedoc:begin -->` markers that implied an owner it did not have. `grep -rn "weavedoc:begin" .weavedoc/` returned nothing; `grep -rn "CLAUDE.md" .weavedoc/bin/` returned nothing. The block was written once, at init, and read by nobody since.

**Why a stale pointer is not a typo.** CLAUDE.md is injected into every session *before any file is opened*. The block's parenthetical — `(status filtering, as_of, provenance)` — therefore reached the reader ahead of the file it referred to, and the reader completed the pattern: one session opened `READ.md` first, exactly as the block instructs, and still reported the **v2** protocol as though quoting it, cited a `status: ok` field the mine does not have, built the user's options on that model, proposed an action `validate` rejects as `TRUTH-V2-FIELD`, and — when challenged — said `READ.md` was out of date. It was not; the file was correct v3 throughout. **A wrong model does not merely produce a wrong answer: put a question to the user on top of it and the error is laundered into a decision**, which is how two unnecessary edits to original source material entered that session looking consented-to. The same block also told the reader to run `bash .weavedoc/bin/weavedoc pull` — an entrypoint deleted in `2026-08-05.3`, so the recommended command had not existed for three months.

**The block now has an owner: the bundle.** Its text ships as `.weavedoc/templates/claude-block.md` — one copy, hashed in the release manifest — and `weavedoc-init` copies that file verbatim (the markers are *in* it, so planting is a copy and never an assembly). `validate` byte-compares the marked region against it, CRLF-normalised, and warns `CLAUDE-BLOCK-STALE`; `CLAUDE-BLOCK-NOTEMPLATE` fires when the template is missing, because an un-run comparison must never read as a pass. Both are **warnings**: a stale pointer is not a mine-integrity failure and may not block a ship. Silent when neither marker is present — a project that never planted the pointer has no pointer to be stale. **The machine does not rewrite the block**, and that is deliberate: the mine's writers do not reach outside the mine, so the repair is re-running `weavedoc-init` (its reconfigure path already re-ensures this exact artifact). UPGRADING.md now says so at the point of the migration that exposed it.

**The block is a pointer, and a pointer may not summarise.** The `(status filtering, …)` parenthetical is gone, and the rule replacing it is stated where the block is written: a summary of a *versioned* document is a summary that must eventually lie, and — sitting in CLAUDE.md — it wins against the eyes of whoever opens the original next. What replaces it is one line telling the reader not to recall READ.md from memory, which is version-independent and counters the failure that was actually observed. The same sweep found the general shape of the defect and named it in `weavedoc-init`'s Rules: **what init PLANTS may not carry version-dependent prose**, because a bundle file travels with the bundle and a planted file does not. One more instance fixed under that rule — `templates/project.md`'s `authority` comment said "legacy-inert (schema v3)" in a file copied into the mine once and never revisited.

**Checks.** `doccheck` gains two: the shipped block must exist, carry both markers, name `.weavedoc/READ.md`, and **not name `status`** (a TEXT check, named as one — it proves the vocabulary that caused the incident is gone, never that no summary can return); and no live surface may invoke `.weavedoc/bin/weavedoc <cmd>`, the deleted bash entrypoint — matched with a trailing space so README's historical mention of the file by name stays legal, and with `notes/`/`CHANGELOG.md` excluded on purpose, since rewriting a recorded measurement to satisfy a grep is falsifying the record. Eight regression cases cover absent · current · stale · CRLF · broken marker pair · **duplicated block** · missing template · the `--json` warning surface; five mutations (drop the CRLF normalisation, silence the missing-template branch, require a well-formed pair before warning, drop the duplicate branch, drop the comparison) each kill their case. The duplicate branch is the same hole one level down: a rewrite reaches only the first marked region, so a second copy below it is a pointer no repair touches and — if the first one matches — no comparison sees, while the session reads both.
...
**map carries the v3 loop and the tag discipline.** Step 4 records a disagreement as a conflicts.json entry through the CLI (three shapes, total: challenger-vs-card · card-vs-card with both claims as self-contained candidates · no-settled-current with `targets: []`), and cards never wear it. Step 5 is the five human rulings and their applies (keep · adopt-in-place · corroborate · split-by-hidden-axis, which is where v2's 병기 went · reject-all), each ending in entry deletion — resolution IS deletion, and re-detection compares fresh. The correction table loses its status vocabulary: withdrawal = delete + `removed:` line (the allocator never re-grants, so no tombstone), retracted sources mean re-ground or remove (`TRUTH-SOURCE-DANGLING`), ids come from `alloc truth` only. And the **§3 tag discipline** lands as four rules (read `tree.md`'s list first · reuse similar tags · ask when ambiguous · re-check on edit) with the boundary stated: the machine narrows by tag and never interprets one; "same fact?" is judged inside the neighbourhood, by the AI. **doccheck check 7** pins all four sentences (acceptance test 19; mutation-verified green→red→green — removing a rule names it).

**The other skills stop teaching dead machinery.** write's conflict guard reads the open lane instead of card statuses; review's A0 and deterministic floor name the store, `CONFLICT-OPEN` and the dangling tripwires; gather routes retraction to re-ground-or-remove and `dated` stops being a resolution axis (context for the human, never an ordering rule — nobody, machine or AI, resolves by date now); init loses the attribution interview question (병기 is the 분리·병합 ruling, per entry — the config key stays legal for migrated mines, unread); gaps counts over cards that exist and treats open-lane values as pending, not missing; verify's delta speaks added/edited/removed; reviewers' T5 gets v3-native consumer failures (a basis that left the mine; a contested card cited as settled because only the open lane shows the dispute).

**Leftovers the grep caught beyond the skills:** WORKFLOW §5's `decision_kind` guard (now `provenance: adopted` carries that trace) and §6's validate/census/pull rows re-derived from the running v3; FORMATS' `authority` and config `attribution` marked legacy-inert with the reason (no ruling is ever by role rank; the keys stay legal so migrated mines validate), `corrects` re-pointed at re-grounding, the changelog example rewritten to v3 kinds, the attest-tombstone sentence dropped; METHODOLOGY §5.2/5.3 rewritten to the three-line division (AI detects · human rules · machine keeps the ledger; the split IS the record); README's dead `N tombstone` legend bullet deleted and attest's refusal scoped to retracted materials; `REQTAG-EMPTY`'s message and cmd-status's dangling v2 comment fragment cleaned. Suite unchanged at 553/553 — no contract moved; the describers caught up to it.
...
**The transform is total over the v2 status axis.** ok cards keep their file and lose exactly the four v2 lines (byte-exact strip, no reserialization); discarded/retracted cards are deleted and their MACHINE-ledger rows go with them — the verify sidecar rows and, learned from the rehearsal below, their coverage.md mentions (scrubbed per-line: a bullet whose every truth id left is dropped, a multi-id bullet is trimmed); conflict cards move losslessly into conflicts.json (surviving `ok` partners as targets, all-conflict components as `targets: []` — migration MOVES an undecided disagreement and never resolves one, so post-apply validate is red by that entry alone, by design). `superseded` is a winner's field and never a deletion class. Three stops land BEFORE the first write: `unsupported` cards (silent canonical promotion), authorized 병기 (`resolution.type: attribute` — "both are right" always names a hidden axis; write it into the claims), and documents citing cards about to leave (a dangling citation is the exact corruption the id discipline exists to prevent). `decided_by: machine` picks are reported, never blocked. The allocator seeds from a pre-deletion high-water scan over every mine-owned surface — a document citing t073 with no such card still lifts the counter past 73.

**The verify is the warranty:** one conservation equation (v2 cards = kept + deleted + moved, zero loss, zero duplication) and one EXACT validate expectation (only the predicted CONFLICT-OPEN, or clean when nothing moved) — anything else fails the migration and prints the restore words. Preflight predicts the two ways that expectation could surprise (orphaned required_tags, cited leaving cards) so apply never discovers them late.

**The rehearsal on the real mine (an eclypse copy, runtime bundle replaced the deployment way with config preserved): 274 cards → keep 248, delete 26, move 0** — and the first run FAILED its own verify with 12 dangling coverage mentions, which is the exact-expectation doing its job; the coverage scrub above is that finding, mechanized and pinned in the suite. Second run: conservation holds, validate fully green, census/pull/status all answer, the human ledgers byte-identical, allocator next t278/m33/c1. Nine new cases (end-to-end with byte-level strip assertions · dirty/no-git refusals · all three preflight stops · ok-partner targets · read-only --check · the verify naming an unexpected problem), eleven targeted mutations, ten killed — the survivor (removing the conservation equation) re-counts the transform's own loop, unreachable by any legal input, and is documented in the case rather than hunted with an artificial one.
