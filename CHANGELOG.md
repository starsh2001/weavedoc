...
- **A single-line `<!-- … -->` holding a violation kind, followed by prose on the same line, is now a `REVIEW-COMMENT-SWALLOWS` incident.** The rule was always "a deliberate archive ends its line with `-->`"; the old reader spliced same-line comments out before the tripwire could see them, so only the multi-line form was ever tested. Both forms are judged now. This can newly block a `review.md` that passed before; the repair is the one the diagnostic already names — put the closing `-->` on its own line, or write the kind without brackets.
