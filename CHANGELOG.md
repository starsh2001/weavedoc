...
**키 이름이 `authority`가 아닌 이유는 실측이다.** `project.md`에 이미 `authority`가 있다 — v2 role rank의 잔재로 legacy-inert지만, **템플릿이 `authority: []`를 심어서 기존 광산마다 그 줄이 이미 있다.** 같은 이름을 쓰면 그 줄이 새 축의 값처럼 읽힌다(그리고 `[]`는 세 값 중 아무것도 아니다). 한 낱말이 두 뜻을 갖는 것은 이 저장소가 반복해서 지워 온 결함의 거울상이라, 이름을 갈랐고 잔재는 건드리지 않았다.
