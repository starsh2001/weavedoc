...
- `status` — enum: `collected` | `converted` | `verified` | `used` | `retracted`. `collected` = brought in but **not yet in the extraction pool** — `map` reads only `converted`/`verified` materials, which is what makes this the visible carrier of "registered but unratified": a `delegated` mine's machine-registered conversation materials sit here until their batch approval converts them (gather's Authority row), so an unapproved draft can never ground a `stated` truth. `verified` = passed cold verification (conversion fidelity confirmed). `used` = a consecrated document cited it (stamped by `refine`) — lifecycle, **not** a verification verdict: the stamp overwrites `verified`, so `scope` counts a used-but-unverified material as still owed. `retracted` = withdrawn by the user (set by `gather`'s retraction flow) — the material grounds nothing from then on: every truth grounded on it is re-grounded on a live material or removed (map's job; `validate` names each leftover as `TRUTH-SOURCE-DANGLING`), but the folder and files stay as audit trail — **never deleted**. This field is a **mirror of the evidence, not a second judge of it**: `validate` blocks `MAT-STATUS-LEDGER` when the mirror and the verify-ledger's winning verdict cannot both be right — still `collected`/`converted` under a recorded `verified`, or `verified` over a recorded `failed`. The axes stay split (the lifecycle overwrites `used`/`retracted` are legal, and `verified` with no ledger row is legal v1 history); only the disagreement is named, and whether the bytes still match is `scope`'s to say.
...
Three duties ride these two line types (the full rules live in `weavedoc-gather`, steps 3 and 7). A machine turn a user answer **leans on** — an accepted proposal, an answered question, the antecedent of a demonstrative — is **carried** into the note layer as a `> [machine-note]`, labeled machine speech, never woven into the user's words: `source.md` is behind the search shield, so a value whose only antecedent lives there is, to every protocol-following reader, grounded nowhere. A figure either line presents as **measured** (실측) carries its measurement date and the exact command that produced it — these lines are exempt from the mirror checks, so nothing downstream ever re-counts them. And a `conversation`/`user-answer` body ends with gather's write-audit line, `> [machine-note] 쓰기 대조 <date> — 본문 N줄 대조 N/N · note 사실 진술 N건 원본 확인` — a **claim** of audit, never evidence of one: the cold lane re-checks it, and its absence reads as unaudited.

One inline mark has declared semantics in the body itself: `[기계 제안값]`, beside a value in a **ratified-summary** conversation material (weavedoc-gather §Conversation as a source), says the value originated as a machine proposal and the user ratified it knowingly. It is ordinary body bytes — present in `source.md` and mirrored into converted.md, so the mirror checks see it like any other text — but it is provenance annotation, not content: `map` reads it as `provenance: adopted`, and it never enters a claim. Unmarked ratified text is `stated`.
...
- `declaration` — enum: `declared` | `anchored` | `no-source` | `legacy-unbound`. The first three are what `intake` writes; **`legacy-unbound` was minted only by the retired v2→v3 migrator** (pinned at v0.6.14, commit `924e97e`), exactly as its verify-sidecar twin was — existing rows persist and every reader still counts them apart; nothing mints new ones.
...
- `note` — free text, non-empty: how it arrived (who handed it over, from where), the **ruling** behind `--no-source`, or what an `--anchor-existing` run was vouching for. Migration wrote the origin token `pre-intake-ledger` here.
- `date` — `YYYY-MM-DD`.
...
A mine that predates this ledger carries a backlog of materials bound to nothing: editable, on either side, with no trace. The migrator minted them `legacy-unbound` because that is the only honest thing a migration can say — **nobody witnessed those bytes** — and the backlog then sits there. The way out could not be for the migrator to hash them on its way past: a digest minted by a migration reads afterwards as evidence while in fact recording whatever happened to be on disk the moment a tool ran, *including an edit made ten minutes earlier*. That is how a falsified copy becomes canon, and a real run came within one command of it.
...
The migrator was later corrected to **report** the unbound count and what it costs instead of minting its rows in silence — leaving materials unbound was always a decision, taken on the owner's behalf and never put in front of them, and `legacy-unbound` reads as a verification backlog (old, low priority) when what it means is that nothing will notice if these files change. That report duty now lives where the rows are counted: `scope` and `census` name every unbound material on every run.

**`MAT-UNDECLARED` is a WARNING and never a block**, and the constraint is not a matter of taste: every mine in existence predates this ledger, so a blocking version would redden every project at once and be switched off within a day — the same outcome as not shipping it, minus the trust. (The judgment `CLAUDE-BLOCK-STALE` shipped under one bundle earlier: *a warning, never a problem, and it must not block a ship.*) The retired migrator filled a pre-ledger mine's rows as `legacy-unbound` — **real history that binds no bytes**, counted apart and never silently equal to a declaration. On a backfilled mine, and on any mine born since the ledger, `MAT-UNDECLARED` means what it is supposed to mean: a material that appeared *after* the ledger existed and was never declared.
...
- `provenance` — optional enum (default `stated`): **who authored the value.**
  ...
  - `adopted` — the value originated as a **machine proposal the user adopted**, or was **machine-fetched from an `origin: research` material and accepted**; the adoption exchange must be visible in the source material — the settling exchange a raw-exchange material carries, or the `[기계 제안값]` mark a ratified summary carries beside the value (weavedoc-gather §Conversation as a source). (A real run recorded a machine-proposed 158cm in the exact same shape as user-supplied values — this field is what keeps "기계는 조용히 고르지 않는다" true at the *record* level, not just in chat.) **A truth whose source is `origin: research` may not be `stated`** — nobody stated a value the machine went and got, and since `stated` is the default, silence would land there and undo at the truth level exactly what `origin: research` stops at the material level. `validate` enforces it.
  - `derived` — the machine computed/interpreted it from other facts. Requires `derived_from`; `validate` enforces this.
  `adopted` + `derived` form the **priority re-verify set** — verification returns to machine-originated values first. In the human-confirmation delta the two part ways (the 0.6.13 confirmation-axis ruling): individual confirmation attaches to what the machine decided **alone** (`derived` figures, hardened hedges, machine-made negative propositions), while an `adopted` value the user chose in a visible exchange is listed under its tag and **never re-asked** — the confirmation it was owed was paid in place when it was applied (weavedoc-verify §Human confirmation). Only `strict` re-lists blanket-passed judgment items in the next delta.
...
Rules: truth ids are listed **explicitly, never as a range** (`t003–t007` fails the mechanical check — only t003/t007 are textually present). A `skipped:` line without a reason is an omission. `validate` cross-checks mechanically: every `## m<id>` resolves to a material, every mentioned truth id exists, and **every truth extracted from a sectioned material appears in that section** (adding a truth without updating coverage fails). Materials without a section are legal (legacy — coverage arrives on their next map); T2 treats a missing section as PARTIAL, never PASS. **This ledger, not card presence, is what "mapped" means**: a material with a record (or a `## legacy` entry) is mapped even with zero cards — its facts re-grounded to another material, its value rejected by a ruling, every element skipped with a reason. `census` prints the remainder as `→ unmapped:` (no record, no card cites it) and `→ no coverage record, cards exist:` (mapped before coverage existed — a section or a legacy ruling, never re-mining); consumers take the list from there rather than re-deriving it.
...
**A ruling may be NARRATED here. It may not ORIGINATE here.** No check code reads this file — that is deliberate (the mine log is a human record, never a judgment input; validate and census each carry that rule at the exact spot they refuse to read it), and it has a consequence the rest of this document owes a rule for: a line that says `소유자 판정: …` or `ruled: …` and exists nowhere else is a decision carrying the run log's full authority and none of a ruling's checks. Nothing can catch it, because nothing is looking. Rulings live in the ledgers that ARE read — `coverage.md` (`ruled: <date>` plus the quoted utterance), `questions.md` (`answered`), `truths/verify.md`'s `## Adjudications`, `review.md`'s adjudications block, and the removal of a `conflicts.json` entry — and a changelog line about a ruling names the ledger holding it. A verification round treats a ruling that names none as unattributed, the same verdict a `coverage.md` `ruled:` with no utterance already gets.

Measured, and the root is one level down (eclypse, 2026-08-26): a block recorded `소유자 판정: 아이돌 재즈팝` for conflict `c003` when the user had never said it. The genre had not been among that material's four questions at all — the phrase came from **the machine's own rationale for its recommendation**, written into `source.md` under the heading `기계가 각 추천에 붙였던 근거 (제안의 일부이며, 사용자 진술이 아님)`, and a later run read it back as the user's answer. So the rule this file needs is not only about where rulings live but about what the machine's own prose is: **text the machine wrote to persuade the user is never evidence of what the user decided, and it carries a heading that says so** — that mine invented exactly this heading, which is why the correcting round could measure the forgery instead of inheriting it. Attribution is a property of the ledger a claim sits in, never of how confidently the sentence is written.

## truths/verify.md (verify state)
Written by `weavedoc-verify` (truths mode). Records the verification state of the truth set.

Frontmatter:
- `status` — enum: `passed` | `failed` | `escalated` | `stale` | `in-progress`. `passed` is written **only** when `consecutive_passes` reached `config.verify.repeat` — a clean round that leaves the count short is `in-progress`, not `passed`, because one clean round is evidence about that round and not about the target. `failed` = the last round found blocking findings. `escalated` = `max_rounds` exhausted with the count still short (`max_rounds` budgets the **whole cycle** — every cold round of every phase — since verify's One cycle; the config key is unchanged). `stale` = new truths added since the last pass; re-run needed.
- `round` — int, last round number.
- `consecutive_passes` — int, optional (absent = 0). Clean rounds **in a row** so far, **over unmoved bytes** — counted in the repair-proof phase of verify's One cycle, or in dry collection rounds when the cycle repaired nothing. The config key and default are unchanged; the meaning relocated with One cycle: the alternating loop used to advance this count while repairs moved the target between rounds, which contradicted the count's own premise. +1 on a clean round; back to **0** on any failing round, on a moved baseline (the step-0 pin), on a repair landing (the bytes the count spoke for are gone), *and* on a round whose defender made a **bar-crossing ticket-downgrade** (blocking at the claimed grade, non-blocking at the new one — the ladder in `weavedoc-review`'s reviewers.md): that downgrade's test is the next fresh panel, so the loop may not converge before it runs. Written after every round so a cold session resumes the loop instead of restarting it.
...
- `authority` — optional enum: `strict` | `standard` | `delegated`. **WHO decides**, and the axis is **orthogonal to every other knob here**, which set how hard we *check*: `strict` + `light` ("check shallowly, but the user signs everything") and `delegated` + `full` ("check as hard as possible, the machine rules") are both coherent configurations, not contradictions. Each skill carries its own row of the level table in its `## Authority` section, and a decision no level's text names goes to the user. **Absent means `standard`** and `validate` names only an unfamiliar *value* (`CFG-ENUM`): the axis is younger than the mine, nothing back-fills the key, and a written value differs from an absent one in exactly the way that matters here — whether anyone chose it. A document may override the mine's level in its `plan.md` frontmatter (same three values); the override governs that document's own steps and leaves mine-side work at the mine's level — whether a strict document reaches back over the materials it draws on is **undecided**, and stated as undecided rather than settled by default. The word used to have a second meaning — `project.md` carried `authority` as a v2 role-rank list, legacy-inert under schema v3 — and that corpse was **retired in the same bundle that claimed the word for this axis** (0.6.14): one user, one mine, that mine's frontmatter never carried the line, so there was nobody left to stay compatible for. One word, one meaning.
...
- `conflicts` — source-vs-source conflict handling (detection is review's #1 priority, **never off**). `detection` (`standard` | `deep`) — how hard to hunt conflicts between materials. (There is no attribution knob: 병기 is the 분리·병합 ruling, per entry, always the user's — no standing authorization exists. The v2-era `attribution` key was retired in 0.6.14; a stray line under `conflicts:` is simply unread.)
- `verify` — per-transformation fidelity check (upstream of the document-level review gate). `strength` (1 = block on critical, 2 = + should-fix, 3 = + nice-to-have — default 2), `max_rounds` (the whole cycle's cold-round budget — see `weavedoc-verify` §One cycle, which relocated the meaning without touching the key), `repeat` (**clean rounds in a row** required to pass, counted in One cycle's repair-proof phase over frozen bytes — keyed by scale, `full` defaults to 2; one clean round is not a pass), `scale` (`skip` | `light` | `standard` | `full` — project default; material format overrides per the format-risk table in `weavedoc-verify`).
...
## CLAUDE.md / AGENTS.md (the pointer blocks — bundle-owned)

`weavedoc init` plants a fixed block in the project's `CLAUDE.md` and `AGENTS.md`, between `<!-- weavedoc:begin -->` and `<!-- weavedoc:end -->`. Its text is **the bundle's**, shipped as byte-identical `.weavedoc/templates/claude-block.md` and `.weavedoc/templates/agents-block.md` files — both hashed in the release manifest and copied verbatim (markers included). Project-specific text goes **outside** the markers; the marked region is machine-owned.

Each is a **pointer and only a pointer**: it says a mine is here and that `.weavedoc/READ.md` must be read before any data is, and it never restates what READ.md says. That is a hard rule, not a style preference. The active harness instruction file is injected into every session *before* any file is opened, so a summary living there does not merely go stale — it **primes**, and a primed reader reports the remembered version as the file's content. Observed on a real mine (2026-08-13): the block still carried a v2 parenthetical after migration had moved READ.md and every card to v3; a session read READ.md first, as instructed, reported the *v2* protocol as though quoting it, cited a `status:` field the mine does not have, and built the user's options on that model.

`validate` byte-compares each marked region against its shipped template (CRLF-normalised, so a Windows checkout does not false-alarm) and warns `CLAUDE-BLOCK-STALE` or `AGENTS-BLOCK-STALE` on any difference — blocks are not automatically rewritten, because the mine's writers do not reach outside the mine. The repair is re-running `weavedoc-init`, whose reconfigure path re-ensures both. Each check is silent when neither marker is present in its file and reports the corresponding `*-BLOCK-NOTEMPLATE` when its template is missing, rather than letting an un-run comparison read as a pass.

## .claude/settings.json / .codex/hooks.json (the hook pairs — bundle-owned entries)

`weavedoc init` plants two hook entries in each harness config, copied from `.weavedoc/templates/hooks.json` and `.weavedoc/templates/codex-hooks.json`; both templates are hashed in the release manifest. Claude Code's **lease** hook observes `PostToolUse` matcher `Skill`. Codex has no skill-activation hook event, so every skill runs the side-effect-free `activate <weavedoc-skill>` handshake and Codex's `PostToolUse` matcher `Bash` binds it to the session id. In both harnesses the **gate** hook (`PreToolUse`, matcher `Write|Edit`) refuses a mine write when no lease names a skill that owns every target path, and its refusal says which skill does and how to activate it. Codex `apply_patch` is delivered to this matcher under the `Write`/`Edit` aliases and may contain several targets; the gate checks all of them.

An entry is weavedoc's **exactly when its command contains the marker substring `.weavedoc/bin/hooks/`**. Every other entry in that file belongs to the project — init creates the file and its arrays when absent, replaces marker-bearing entries, and **removes nothing it does not own**. The commands are stable, version-free paths into the bundle, so what the gate does travels with `.weavedoc/`: replacing the bundle updates the rules without a second visit to `settings.json`, which is what keeps these entries inside the planted-artifact rule.

**It is a tripwire, not access control, and the distinction is the whole design.** The gate can see that a mine file is being written with no owning skill loaded. It cannot see *which* skill the work belongs to — that judgment stays with the nine skills' own rules, which is why they carry it in text. It fails **open**: any internal error allows the write and says so, because a crashed gate that bricked every mine write would be switched off within a day, the same end-state as not shipping it minus the trust. A write routed through a shell bypasses it, and so does any tool outside the matcher. A lease is forgeable. What is being defended is **memory** — the measured failure was eight materials and twenty-three cards produced by a session that never loaded the skills owning them — not malice; the intake ledger states the same limit about itself, for the same reason.

`validate` compares each harness's planted entries against its shipped template and warns `HOOKS-STALE` (Claude Code) or `CODEX-HOOKS-STALE` (Codex) on any difference, including a half-planted pair. The comparison is **structural, not byte-for-byte**: these are project files, reformatted by their own editors, so what is compared is the marker-bearing entries as `(event, matcher, command-from-the-marker-onward)` tuples, order-insensitive — the command's prefix is environment while arguments after the script are not. As with the pointer blocks, entries are not automatically rewritten; re-run `weavedoc-init` (reconfigure). A check is silent when no marker-bearing entry exists and reports `HOOKS-NOTEMPLATE` or `CODEX-HOOKS-NOTEMPLATE` when its template is missing.
...
- **Each slot is its own block — bold label, one bullet per item.** The label stands on its own line; under it, every item gets a `-` line (a count, a file, a decision), and uniform items may sit in a small table instead. Slots never fold into a running paragraph: the reader scans the closing for *their* slot, and prose that buries the four labels mid-sentence hides the one line they came for (measured 2026-09-23 — the owner read real closings as "줄글로 주루룩" and could not find what was theirs). A wall is the telegram's twin: both are format violations even when true, and the sentences *inside* each bullet stay complete — what the block shape bans is folding items together, never explaining them.
...
## The decision loop (how a run asks the user to rule)

The step report is how a run *ends*; this is how it *asks* while it runs. It governs every surface where pending items wait on the user's judgment — an open conflict entry (map), a fill-or-accept gap (gaps), an ambiguous tag, a correction ruling, a Human-queue entry. It does **not** govern preference elicitation (plan's want-gathering keeps its own 2–4-per-round batching), the init interview (a fixed questionnaire whose payloads carry several questions by design), or verify's delta confirmation (a rendered list, confirmed as a blanket — 0.6.13's axis). The motive is measured (owner, 2026-09-23): real runs rendered the whole docket as one prose block and closed with "t243부터 어떻게 할지 결정해주시면 됩니다" — every pending ruling at once, keyed by machine ids, the user left to build their own worklist out of a paragraph.

- **One item per ask.** Never render the whole docket and ask the user to answer by id. Order the docket once (blocking severity first, then cheapest fill first — gaps' cost labels are the model), say the order in a line, then ask item by item with a progress marker (`3/7`).
- **Substance first, the id last.** The ask carries the item in words — what the claim says, each side's value with its material's subject and the verbatim quote. An id is a machine handle: it appears once, in parentheses at the end, as the file reference — never as the token the user must answer with.
- **Options say outcomes, and one may wear `(추천)`.** Each option states what would happen to *this* fact ("정정 자료의 2월 14일로 고친다"), never a bare mechanism name ("새값 채택"). Mark at most one `(추천)` with a one-line why; the ruling stays the user's, and a **free-form answer is always open** — the harness's Other field, or an explicit invitation to write one.
- **Use the harness's native question surface** where one exists — Claude Code's `AskUserQuestion` (one docket item per call), Codex's `request_user_input` — and numbered markdown options otherwise.
- **Apply each ruling as it lands**, before the next ask (batch housekeeping — reindex, changelog, propagation — may still close the docket as one pass). A ruling applied immediately is a ruling a stop cannot lose.
- **Stopping is normal, and resume is free.** The user may stop at any point (the stop button, "그만", "나중에"); a stop is a pacing decision, never a failure. Close with the ordinary step report; everything unruled stays open in its store (conflicts.json, `gaps.md` `# Open`, `questions.md`) — which is exactly what makes "계속하자" work: a later run reads `status --open` and re-enters at the first unruled item.
- **Surface-don't-point governs the report; this loop governs the ask.** The closing list of everything still open is the handoff, not the questionnaire. Using that list *as* the ask — the dump-then-"decide from t243" shape — is the measured failure this section exists to end.
...
| `AGENTS-BLOCK-NOTEMPLATE` | *(warning)* AGENTS.md carries the marker block but `.weavedoc/templates/agents-block.md` is missing — the comparison did not run |
| `AGENTS-BLOCK-STALE` | *(warning)* AGENTS.md's weavedoc block is not the one this bundle ships (or its marker pair is broken) |
...
| `HOOKS-NOTEMPLATE` | *(warning)* `.claude/settings.json` carries weavedoc hook entries but `.weavedoc/templates/hooks.json` is missing or names none — the comparison did not run |
| `HOOKS-STALE` | *(warning)* the planted weavedoc hook entries are not the pair this bundle ships (a half-planted pair counts), or the file no longer parses around them |
| `CODEX-HOOKS-NOTEMPLATE` | *(warning)* `.codex/hooks.json` carries weavedoc hook entries but `.weavedoc/templates/codex-hooks.json` is missing or names none — the comparison did not run |
| `CODEX-HOOKS-STALE` | *(warning)* the planted Codex hook entries are not the pair this bundle ships (a half-planted pair counts), or the file no longer parses around them |
...
| `MAT-STATUS-LEDGER` | a material's frontmatter `status` disagrees with the verify-ledger's winning verdict — `collected`/`converted` under a recorded `verified`, or `verified` over a recorded `failed`. The lifecycle overwrites (`used`, `retracted`) stay legal, and `verified` with no row at all stays legal v1 history |
...
| `PLAN-AUTHORITY` | plan `authority` outside its enum (absent = inherit the mine's) |
...
| `VER-V1-BRIDGE` | the mine is schema v1 — migrate via the pinned bridge runtime v0.5.21 first, then the v0.6.14 bridge |
| `VER-V2-UPGRADE` | the mine is schema v2 — migrate via the pinned bridge runtime v0.6.14 (the last bundle carrying the v2→v3 migrator) |
