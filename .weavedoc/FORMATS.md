...
- `authority_level` — optional enum: `strict` | `standard` | `delegated`. **WHO decides**, and the axis is **orthogonal to every other knob here**, which set how hard we *check*: `strict` + `light` ("check shallowly, but the user signs everything") and `delegated` + `full` ("check as hard as possible, the machine rules") are both coherent configurations, not contradictions. Each skill carries its own row of the level table in its `## Authority` section, and a decision no level's text names goes to the user. **Absent means `standard`** and `validate` names only an unfamiliar *value* (`CFG-ENUM`): every mine predates this axis, so requiring the key would redden all of them at once — the same judgment `MAT-UNDECLARED` and `CLAUDE-BLOCK-STALE` carry. `upgrade` does not write it either; a written value and an absent one differ in exactly the way that matters here, which is whether anyone chose it. A document may override the mine's level in its `plan.md` frontmatter (same three values); the override governs that document's own steps and leaves mine-side work at the mine's level — whether a strict document reaches back over the materials it draws on is **undecided**, and stated as undecided rather than settled by default. **Not the same word as `project.md`'s `authority`**, which is a list of roles and a v2 rank corpse kept legal and unread; the names differ precisely because every template-planted `project.md` already carries that line, and one word meaning two things is how a reader lands in the wrong one.
...
| `PLAN-AUTHORITY-LEVEL` | plan `authority_level` outside its enum (absent = inherit the mine's) |
...
| `PROJ-AUTHORITY` | `project.md` `authority` names a role it does not declare — the v2 role-rank corpse, unrelated to `config.authority_level` |
