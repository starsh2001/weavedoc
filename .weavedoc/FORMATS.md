...
## .claude/settings.json (the hook pair — bundle-owned entries)

`weavedoc init` plants two hook entries in the project's `.claude/settings.json`, copied verbatim from `.weavedoc/templates/hooks.json` — one shipped copy, hashed in the release manifest. A **lease** hook (`PostToolUse`, matcher `Skill`) records which weavedoc skill a session is operating under; a **gate** hook (`PreToolUse`, matcher `Write|Edit`) refuses a mine write when no lease names a skill that owns the target path, and its refusal message says which skill does and how to get there.
...
`validate` compares the planted entries against the shipped template and warns `HOOKS-STALE` on any difference, including a half-planted pair. The comparison is **structural, not byte-for-byte**: settings.json is the project's own file, reformatted by its own editors, so what is compared is the marker-bearing entries as `(event, matcher, command-from-the-marker-onward)` tuples, order-insensitive — the command's prefix is environment (an install that had to spell an absolute path is the same wiring) while arguments after the script are not, because those change what runs. As with the pointer block, the entries are **not** automatically rewritten — the mine's writers do not reach outside the mine — and the repair is re-running `weavedoc-init` (reconfigure). The check is silent when no marker-bearing entry exists (a project that never wired the gate has nothing to be stale) and reports `HOOKS-NOTEMPLATE` when the template is missing, rather than letting an un-run comparison read as a pass.
