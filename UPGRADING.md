...
## 프로젝트 CLAUDE.md의 포인터 블록

브리지에서 이주를 마치고 이 런타임으로 돌아오면, `init`이 심은 `<!-- weavedoc:begin -->` 블록은 여전히 이전 번들의 것입니다. `validate`가 `CLAUDE-BLOCK-STALE`로 알려 주며, 수리는 `weavedoc-init` 재실행(reconfigure)입니다. 마커 바깥의 프로젝트 고유 문장은 그대로 남습니다.
