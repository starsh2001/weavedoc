...
> **Decisions: one at a time, in words, with a way out.** When pending items wait on the user's ruling — a conflict, a fill-or-accept, an ambiguous tag, a Human-queue entry — run **the decision loop** (`.weavedoc/FORMATS.md`, "The decision loop"): one item per ask with a progress marker (`3/7`), the item rendered in words (what the claim says, each side and its source) with ids last as the reference, options that say what would happen, **your recommended option marked `(추천)`** with a one-line why, and **always a free-form answer** — never a forced closed pick, never the whole docket rendered as one ask ("t243부터 결정해주세요" is the measured failure). Apply each ruling as it lands; stopping mid-docket is normal, and the open stores are what let "계속" resume at the first unruled item.
...
> **Codex activation handshake.** In Codex, before this skill's first file write, run `node .weavedoc/bin/weavedoc.mjs activate weavedoc-init`. The command itself writes nothing; the project `.codex/hooks.json` PostToolUse hook binds the current session to this skill. On a first init the hook file does not exist yet, so the handshake takes effect after `.codex/hooks.json` is planted and trusted; until then there is no Codex gate to evade. Claude Code needs no handshake because its `Skill` event carries the skill name directly.
...
Run the form for the active harness:

```
node .weavedoc/bin/weavedoc.mjs interview claude
node .weavedoc/bin/weavedoc.mjs interview codex
```

Each form prints **two JSON arrays** — Q2 (`authority`, `completeness`, `conflicts.detection`) and Q3 (`verify.strength`, `review.strength`, `scale`). In Claude Code, paste each array verbatim as `AskUserQuestion`'s `questions` argument. In Codex, pass each array verbatim as `request_user_input`'s `questions` argument. One call per array: both interfaces accept this 3+3 split. If the active Codex surface has no interactive question tool, present one decoded batch in chat and wait for the answer before presenting the next; the questions and options do not change. `authority` opens the first call because it is the framing question — **who decides**, as against the *how hard do we check* axis every other question sets; the two are orthogonal, and a project can want either combination.
...
- **A label maps to one config value** (`strict`, `standard`, `delegated`, `off`, `required`, `deep`, `1`, `2`, `3`, `light`, `full`, `skip`). Claude's label is the value verbatim. Codex requires its recommended label to end in the exact UI suffix ` (Recommended)`; strip that suffix and only that suffix before writing the value. A regression case compares both forms against `.weavedoc/schema`, both directions, so neither questionnaire can offer a value `validate` rejects or withhold a legal one.
...
Write the answers to `.weavedoc/config.yaml`. On a **reconfigure**, update config, then re-ensure the **five** idempotent guard classes from §3 — the **configured folders and their `.gitkeep` markers**, the **search shield (`.ignore`)**, the **evidence byte guard (`.gitattributes`)**, the **CLAUDE.md + AGENTS.md pointer blocks** and the **Claude + Codex hook entries** — and **stop here**. (This said *two* through v0.5.20 while the folder bullet below already claimed to run on reconfigure — the two lines contradicted each other, and a reconfigure that renames a path leaves a NEW empty directory that Git cannot carry, so the marker guard belongs in this list. External review, v0.5.21.) Each class runs on reconfigure too: a reconfigured mine that skips either harness artifact is safe only in the other harness, not dual-platform.
...
- **Search shield (`.ignore`).** *Runs on **reconfigure** too.* Ensure the project root has an `.ignore` file shielding the raw layer from content searches (create if absent; if present, ensure the two entries exist). Use the **configured** `paths` values, not the literals:
  ...
  This is the mechanical half of the read protocol: the CLAUDE.md / AGENTS.md blocks below tell a session *not* to read raw sources; this makes a content search *unable to find them* even in a session that never reads the block. Ruled 2026-07-31.
...
- **Evidence byte guard (`.gitattributes`).** *Runs on **reconfigure** too.* Ensure the project root has a `.gitattributes` pinning the mine's line endings (create if absent; if present, ensure these entries exist — anything the project wants for its own files stays untouched). Use the **configured** `paths` values, not the literals:

  ```
  # WeaveDoc evidence byte guard — a checkout must not rewrite what a digest covers.
  # Generated mine text is pinned to LF, so a clone on any platform reproduces the
  # bytes the ledgers recorded. The raw layer is exempted the OTHER way: `-text` is
  # no conversion in either direction, because an original is the bytes the user
  # handed over and a container original (docx/hwpx/xlsx) is a binary zip.
  # Last matching line wins — the exemptions come last on purpose.
  ...
  **Planting it does not repair a worktree that already drifted, and a plain checkout is not enough.** Measured (git 2.46, `core.autocrlf=true`): with the attributes in place `git ls-files --eol` still reads `i/lf w/crlf`, and `git checkout -- .` then repairs only the `-text` paths — a stat-clean LF-pinned file is left exactly as it is. Deleting the affected paths and checking them out again brings every one back to `i/lf w/lf`. That works *because* the stored blobs were LF all along, which is the thing to confirm first: check that the Git-stored bytes still match the recorded fingerprint, then re-materialise, then re-run `scope`. **Never re-certify the changed files wholesale to clear the alarm:** a real content change looks identical from the outside, and in that same eclypse measurement one card (t088) genuinely had moved.
- **Harness instruction pointers (read-protocol tripwires).** *Unlike the rest of §3, this bullet runs on **reconfigure** too (§2 points here).* Ensure `CLAUDE.md` carries **`.weavedoc/templates/claude-block.md`** and `AGENTS.md` carries **`.weavedoc/templates/agents-block.md`**, each verbatim. Both templates already hold the `<!-- weavedoc:begin -->` / `<!-- weavedoc:end -->` markers and are byte-identical by contract. Create either instruction file if absent; if its markers already exist, replace that whole region (markers included), otherwise append — **idempotent, never duplicated**. Anything the project wants to say for itself goes *outside* the markers. This makes future Claude Code and Codex sessions — including creative ones that never invoke a weavedoc skill — hit the read protocol before touching the mine.

  **Copy; do not retype, reword or re-wrap.** Each block's text belongs to the **bundle**, not to this skill and not to the project: `validate` byte-compares each marked region against its template (CRLF-normalised) and warns `CLAUDE-BLOCK-STALE` or `AGENTS-BLOCK-STALE` when it differs. Those warnings stand between a bundle upgrade and pointers that quietly keep describing the previous one; repair by re-running this skill (reconfigure).

  **A pointer may not summarise.** The block names where the protocol lives and stops; it must never restate what `.weavedoc/READ.md` says. Harness instruction files are injected into every session *before* any file is opened, so a summary there does not merely go stale — it **primes**, and a primed reader reports the remembered version as the file's content. Measured (eclypse, 2026-08-13): the Claude block still carried a v2 parenthetical — `(status filtering, as_of, provenance)` — after `upgrade --apply` had moved the schema, the validator, READ.md and 271 cards to v3. A session read READ.md first, exactly as instructed, and still reported the *v2* protocol as though quoting the file, cited a `status: ok` field that exists nowhere, built the user's options on that model, and — when challenged — blamed the file for being out of date. The same priming mechanism applies to AGENTS.md.

- **Harness hook entries (the skill gate's wiring).** *Runs on **reconfigure** too.* Ensure `.claude/settings.json` carries `.weavedoc/templates/hooks.json` and `.codex/hooks.json` carries `.weavedoc/templates/codex-hooks.json` — **copy the marker-bearing groups verbatim; do not retype, reword or re-shape them.** An entry is ours exactly when its command contains `.weavedoc/bin/hooks/`: replace every group carrying that marker with the matching template's current groups, and **preserve every group that does not**. Create either file, its `hooks` object and missing event arrays when absent — idempotent, never duplicated. Codex project hooks require a trusted project and one-time review in `/hooks`; tell the user to review the planted pair before the next writing step.

  The entries' text belongs to the **bundle**: `validate` compares each harness's planted groups against its template and warns `HOOKS-STALE` / `CODEX-HOOKS-STALE` when they differ (`*-NOTEMPLATE` when the template itself is gone). Repair by re-running this skill. Each command is a stable, version-free path into the bundle, so replacing `.weavedoc/` changes the gate without another visit to either harness file.
...
- **What init PLANTS may not carry version-dependent prose.** A bundle file travels with the bundle — replace `.weavedoc/` and it is current again. A planted file (`project.md`, `.ignore`, either instruction block) is copied into the mine once and **no command reaches back into it**: a migration or bundle re-copy moves the schema, validator and READ.md while planted prose stays. So a planted artifact states only its own content and points at the bundle for everything else. The two instruction blocks and two hook files are bundle-checked because the former are injected before any file is read and the latter run without a session reading them.
