...
**A finding missing its ticket files one grade lower** (missing the nice-to-have ticket = dropped). This is a *formal* test the defender may finalize on its own at `standard` and `delegated` — at `strict` even this goes to the user's list, because there the defender finalizes nothing (the triage section's level hook governs, and this sentence sat above its scope until a cold review caught a strict-forbidden drop riding through it). Unlike a downgrade for **semantic** reasons, which still goes to the Human queue (see triage below). Grades are argued by ticket, never by adjective. The gate's kinds (contradiction/unsupported/missing-required) have no grades and no tickets — they are facts, outside this ladder. A ticket-downgraded finding is **not** condensed into do-not-raise — it stays open at its new grade, and the next round's fresh cold panel may re-raise it *with* the ticket. A finding that keeps failing to produce its ticket across fresh cold rounds has found its true grade. That sentence presumes a next round exists, so one guard makes it true: a ticket-downgrade that **crosses the blocking bar** (the finding would block at its claimed grade and does not at its new one) leaves the round **not clean**, so the loop cannot converge on it and the next fresh cold panel always runs. Nothing is routed to the human at `standard` and `delegated` (at `strict` the downgrade itself already went up) and the grade is not re-argued — the fresh panel either re-raises the finding with its ticket, or a grade that survived two independent cold reads stands.
...
**The defender's authority to FINALIZE follows the mine's authority level** — read `authority` from `.weavedoc/config.yaml` (document work: the `plan.md` override), absent = `standard`. Everything below describes the `standard` level, which is also what the verify/review skills' Authority rows say. At **`strict`** the defender finalizes **nothing**: it still runs and still writes its verdict recommendations, but every finding — formal drops, the evidence-bar close, `[machine]`-taggable hygiene — goes to the user's list, and `nice-to-have` findings are reported rather than routed away. At **`delegated`** it holds full triage authority including rejection, with the round summary as the user's view. The level moves who finalizes, never what is true: quote-existence failures and fidelity violations are facts at every level.
...
- **Drop:** anything out of scope (per the SCOPE rule above); anything failing the "name the consequence" test; anything in the do-not-raise categories; duplicates.
  ...
  **A do-not-raise category never outranks a spec rule that leaves no discretion.** Adjudications are case law a mine accumulates; the SKILL files are the rule. When a finding falls under both — an adjudication that would exempt it AND a rule that admits no exception (gather's "Nothing lost … Missing content is a defect, not a summary") — the rule decides and the verdict is KEEP, not Human queue. Say which rule won in the entry. Without this, a mine's own case law quietly narrows the spec, one category at a time (measured: a defender escalated a value the mirror rule flatly requires, because a note-compression category happened to cover the spot it sat in).
...
**One of those three reasons is checkable, and therefore closable — but only when the showing is complete.** "다른 파일에 기록돼 있음" is on the list because it kept being asserted from *memory*, not because it is a judgement: unlike the other two, it is a proposition `grep -F` settles. The defender may close a finding for that reason when it presents all three of —

1. the ruling **quoted verbatim**,
2. the **file and line** it sits at, and
3. the **command that finds it, with its non-zero count**.

It then writes the entry `[ruled] [machine]` with that evidence inline, exactly as a user ruling would be recorded. Anything short of the full showing stays `[open]` — a partial showing is the memory case this rule was written against. This is the reviewer-side bar pointed the other way: a reviewer claiming a ruling is ABSENT must already measure every file rulings live in (`verify.md` · `changelog.md` · `questions.md`) or the finding does not stand, and requiring measurement for absence while refusing it for presence sends the user a decision list whose answers the mine already holds. Measured: a round escalated two `[recommended]` entries, one of which the defender had itself located in `changelog.md` and quoted **in the very entry it was escalating**. No new discretion is granted — without the full three-part showing the entry goes up exactly as before.
...
**Verify's collection phase hands off a found-list, not classifications — by design.** Inside `weavedoc-verify`'s One cycle, a collection round (its step 1) ends by appending its surviving findings to the state file's found-list — untriaged, ungraded, unrepaired — and the classify-everything gate above fires at the phase boundary instead: the full-set triage that follows collection, and every proof-round return to it. What the gate protects survives whole: nothing falls through between rounds, because the found-list IS the between-rounds record, and no repair or proof round starts until triage has classified every entry. Review rounds, and any round outside that cycle, keep the per-round gate unchanged.
...
5. **consumer-reader** (T5) — **read the mine exactly as `.weavedoc/READ.md` tells a consumer to, and no other way.** Read-scope = everything READ.md points a consumer at, and only that: `node .weavedoc/bin/weavedoc.mjs pull <tag>`, **the truth files pull points at** (rule 2), and **`truths/index.md` / `truths/tree.md` / `census`** (rule 5 — the entry points for finding things by tag), plus `project.md` `required_tags` so you know which topics a writer would pull. A reviewer barred from these would be *weaker* than a real consumer, and would judge a mine nobody actually reads. **Out of scope: materials, `source.*`, coverage, conversion history, how anything was produced.**
