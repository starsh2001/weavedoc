...
> **Codex activation handshake.** In Codex, before this skill's first file write, run `node .weavedoc/bin/weavedoc.mjs activate weavedoc-refine`. The command itself writes nothing; the project `.codex/hooks.json` PostToolUse hook binds the current session to this skill. If Codex reports the hook as untrusted, stop and ask the user to review it with `/hooks`; never route the write through a shell to evade the gate. Claude Code needs no handshake because its `Skill` event carries the skill name directly.
...
> **Write-scope.** This skill writes to `documents/<doc-id>/draft.md`, `review.md` (adjudications), `final.md`, and `questions.md` (the ask loop) — plus, only at the consecration moment in step 9: `status: done` on this document's `plan.md`, `status: used` on the materials that contributed, and the refreshed status column of `catalog.md` (it is generated, and nothing else regenerates it at that step). It does **not** otherwise modify `truths/` or `materials/` — if a fidelity violation reveals a **truth extraction error** (the truth itself is wrong, not just the draft's use of it), **stop and route to `weavedoc map`** to fix the truth. Don't patch the draft to work around a bad truth.
...
**This skill's rows — two, because refine both writes and consecrates.**

*Writing.* `strict` — verbatim quotation only and sentence-level approval, as in write. `standard` — the machine applies the findings and the user's gate is the review round. `delegated` — free editing, citation and grounding rules untouched.

*Document management.* `strict` — a consecrated document is **not edited**: a change is a new version, and a `stale` document is re-verified in full and re-signed by the user. `standard` — `stale` → the machine re-checks and updates, and the user confirms the delta. `delegated` — the machine edits freely; only deletion is the user's. At every level the Human-queue go-ahead before consecration stands unchanged: that gate exists because consecrating over an unread queue *is* the semantic judgement the queue intercepts, and no level makes that judgement the machine's.
...
## Report
Open and close this run with the **step report** — the fixed shape in `.weavedoc/FORMATS.md` ("The step report"): the `[weavedoc-refine <doc-id>] starting` anchor and 2–4 sentences, then `done — …` carrying `Result` · `Open` · `Your turn` · `Next`, in that order, none omitted. `Next` is the section below. What this skill puts in the two middle slots:
- **Open** — findings you did NOT apply and why, fidelity violations still standing (the gate stays shut until they are zero), and the convergence count if the loop has not closed.
- **Your turn** — every `# Human queue` entry, and the decision to consecrate; a clean gate makes consecration possible, it does not order it.
