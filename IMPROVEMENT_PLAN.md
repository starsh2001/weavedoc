...
> 상태(2026-08-08 갱신): **개선 열차 완료 — v0.5.0 발행, bash 런타임 삭제, 런타임은 Node 하나.** Phase 0~5 전부 완료; 이후 이력은 이 줄이 아니라 **CHANGELOG.md(번들 라벨별)와 §11 결정표**가 정본이다. 최신 발행본은 **v0.5.21(번들 `2026-08-07.10`)**이고, 현행 작업본은 **미발행 번들 `2026-08-08.1`**이다. 작업본은 v0.6 후보 ③·④를 하나의 byte-domain Markdown scanner, typed ledger state, source-offset writer postcondition으로 해소하며 세 장부·review·Verified units·coverage와 모든 소비자를 그 모델로 이관한다. 이전 v0.5.x 세부 이력은 CHANGELOG와 git 이력에 보존한다. 릴리스 기준은 §11 결정(태그를 막는 결함은 데이터 파괴·정상 경로 오차단/오통과뿐 — §9의 전체 ShellCheck 통과 등 품질 조건은 백로그이지 태그 게이트가 아니다); 이후 리뷰 발견은 그 기준으로 분류부터 한다. 남은 v0.6 후보는 광산 mutation lock 완전판이다.
>
