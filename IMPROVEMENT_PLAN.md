...
> 상태(2026-08-08 갱신): **개선 열차 완료 — v0.5.0 발행, bash 런타임 삭제, 런타임은 Node 하나.** Phase 0~5 전부 완료; 이후 이력은 이 줄이 아니라 **CHANGELOG.md(번들 라벨별)와 §11 결정표**가 정본이다. 최신 발행본은 **v0.6.0(번들 `2026-08-08.24`, schema v3)**이다 — 그 범위와 계획은 이 문서가 아니라 **NEXT_IMPLEMENTATION_PLAN.md**(2026-08-12 범위 재결정으로 재작성된 v3 최소핵 계획)가 정본이며, 이 문서의 v0.6 후보 ③·④ 서술은 그 재결정 이전 것이다. 이전 v0.5.x 세부 이력은 CHANGELOG와 git 이력에 보존한다. 릴리스 기준은 §11 결정(태그를 막는 결함은 데이터 파괴·정상 경로 오차단/오통과뿐 — §9의 전체 ShellCheck 통과 등 품질 조건은 백로그이지 태그 게이트가 아니다); 이후 리뷰 발견은 그 기준으로 분류부터 한다. 남은 v0.6 후보는 광산 mutation lock 완전판이다.
>
> 비교 기준: `v0.1.0` (`ff0b726`, runtime `2026-07-27.7`) → 이 표가 작성된 시점의 HEAD `7c199e6` (runtime `2026-08-02.2`). **이 §1 표는 그 시점의 대조 기록이며 이후 갱신하지 않는다** — 현행 상태는 CHANGELOG.md(번들 라벨별)와 §11 결정표가 정본이다.
>
...
관련 구현은 [.weavedoc/bin/weavedoc.mjs](.weavedoc/bin/weavedoc.mjs), 형식 계약은 [.weavedoc/FORMATS.md](.weavedoc/FORMATS.md), 현재 QA 기록은 [CHANGELOG.md](CHANGELOG.md)에 있다.
...
> **§11(2026-08-06 결정)이 이 절보다 우선한다 — 태그 게이트는 §11이다.** 아래 목록은 **품질 백로그**이고, 태그를 막는 결함은 §11의 두 종류(데이터 파괴 · 정상 경로 오차단/오통과)뿐이다. 이 화해 문장은 §3 상태 줄에만 있었고 이 절 본문에는 없어서, 두 곳을 따로 읽으면 모순으로 보였다(외부 리뷰 2026-08-06). 예: 아래 "ShellCheck 통과"는 미충족 상태로도 v0.5.x가 발행됐고, 그것은 위반이 아니라 §11이 정한 분류다.
...
| v0.4.0 이후 릴리스 경로 (태그-런 외부 리뷰 후속) | **v0.5.0 직행 — 중간 0.4.1 없음.** 창 안 순서: ① bash가 트리에 있는 동안 포트 회귀(retag·upgrade 쓰기 경로)와 fingerprint·회귀 캐시 키 수정 — 성공 경로 바이트 불변이라 기존 저울로 재채점 ② 장애 주입 검증(읽기 전용 대상·N번째 쓰기 실패·rename 실패·rollback 실패) ③ 마지막 파리티·장애 주입 결과를 별도 커밋으로 고정한 뒤 bash 삭제 + 도구 전환(regress 기본값·doccheck·manifest·파리티 도구) ④ 공통 장부 파서와 attest·Accepted·seal-review·consecrate·reindex 경화 ⑤ 3-OS 검증 후 태그 | 기록된 "0.4.0 다음 릴리스에서 bash 삭제"(README·CHANGELOG·이 표)와 정합 — 0.4.1을 만들면 삭제 연기라는 새 결정이 필요하고 Node-only 경화를 다시 쪼개야 한다. 경화는 새 동작이라 bash 동결과 충돌하므로 삭제와 같은 릴리스에서만 자유롭다. 기각: 0.4.1 선행(연기 결정 필요) · 경화를 0.4.1에 포함(동결 위반 또는 선언 갈림 증식) | 결정 2026-08-05(사용자, 외부 리뷰 수렴) |
| 장부 `LAST row per id wins`와 malformed 행 | **ID 격리를 구현한다 — 문서를 약화하지 않는다.** id별 물리적 마지막 행이 정상이면 그 행이 승리. id별 마지막 행이 malformed면(id 식별 가능) 그 id **전체 격리** — 이전 유효 행 포함 어떤 증거도 인정하지 않고 v1 frontmatter 폴백도 열지 않는다. id조차 식별 불가면 파일 수준 malformed로 validate 차단. 중간의 malformed 행은 현행대로(가시 격리 + validate 차단, 그 뒤의 유효 최신 행이 승리) | 실측(2026-08-05): 현행은 양 런타임 공히 "LAST **valid** wins"라 FORMATS의 "LAST row per id wins"와 어긋나고, 새 판정을 쓰다 행이 깨지면 **과거의 verified가 되살아나** scope가 현재 상태를 잘못 말한다. 이 규칙은 위 2026-08-04 "격리 = 증거 없음(폴백 금지)" 결정을 verdict-불능 행에서 **구조-불능 마지막 행**으로 확장한 것 — §11이 한 방향을 가리킨다. 기각: 문서를 "LAST valid"로 정정(관측된 구현에 맞춘 fail-closed 약속 약화) | 결정 2026-08-05(사용자) — v0.5.0 공통 파서에서 구현, red-first 케이스와 함께 |
| 장부의 CRLF와 미종결 마지막 행 (공통 파서) | **v0.5.0에서 CRLF 허용 — validate·scope·attest가 한 파서를 쓴다.** 줄 끝 CRLF는 LF와 동등. 필드 안 제어 바이트(TAB·CR·LF)는 구조 위반. **내용 있는 미종결 마지막 행은 무시도 통독도 아닌 malformed** — attest가 도중에 죽은 흔적이 그대로 fail-closed 신호가 된다 | 실측(2026-08-05, 컨테이너): CRLF 장부에서 Linux bash scope와 포트 scope가 갈리고(0 vs 1 verified — MSYS bash는 포트와 같아 bash의 단일 답이 없다), 포트 내부에서도 scope(CR 벗김)와 validate(CR 유지, bash `read` 재현)가 반대 판정. **미종결 마지막 행도 scope는 읽고 validate는 버린다** — 같은 파일에 리더가 셋. autocrlf 체크아웃은 Windows의 정상 상태라 관용이 옳고, 포트의 기존 CRLF 철학("벗기는 쪽으로 통일")과 정합. **같은 가족의 인스턴스 둘을 콜드 리뷰가 추가 실측**(2026-08-05, 둘 다 이 패치 이전부터 — 런타임만 HEAD로 되돌려 재실행해 확인): ① CRLF **truth** 위에서 `retag`가 갈린다 — bash validate는 CR을 유지한 채 인용 봉인을 맞춰 `SEAL-QUOTE-MISSING` → 롤백 rc 1, Node validate는 벗겨서 통과 → 커밋 rc 0. 트랜잭션이 검증 갈림을 **commit-vs-rollback으로 증폭**하는 자리이고, `pass_crlf_retag`는 CR 생존만 봐서 **반대 이유로 양쪽 다 통과**한다. ② Node `writePreservingEol`은 빈 줄의 CR을 보존하지 않는다(`l === ''` 스킵). 둘 다 공통 파서 작업 범위에 명시적으로 포함할 것. **bash 동결 중에는 파리티 위반이므로 반드시 bash 삭제와 같은 릴리스에서.** 그 전까지는 (광산,명령) 정밀 예외(기대 델타 고정)로 지탱 | 결정 2026-08-05(사용자) |
| retag·upgrade의 쓰기 경로 (태그-런 외부 리뷰 P0) | **(a)+(b) — 파일 쓰기는 전부 stage+rename, 명령 전체는 snapshot → 전체 시도 → 실패 시 전체 rollback → postcondition 검증.** rollback 실패 시 백업 보존 + 명시 차단("as before"를 주장하지 않는다) | 실측(2026-08-05, 컨테이너): 읽기 전용 project.md에서 bash retag는 post-validate 롤백으로 원상(rc 1), bash upgrade는 rename형 쓰기라 **전체 성공** — Node는 둘 다 EACCES 미포획으로 **반적용 + 백업 잔류**. 원인은 cmd-retag.mjs·cmd-upgrade.mjs가 write.mjs의 writeAtomic(EPERM/EACCES 시 대상 속성 해제 후 재시도 포함)을 우회한 직접 writeFileSync. 주의 둘: writeAtomic은 실패를 **false로 반환**하므로 호출자가 예외로 승격해야 하고(치환만 하면 실패가 조용히 넘어간다), 파일 원자성은 다중 파일 명령의 원자성이 아니므로 rollback 경계는 그대로 필수. 기각: rollback만 추가(안전 실패를 선언 갈림으로 — bash 0.3.x가 해내던 작업을 Node가 거부하는 기능 회귀를 봉인) · bash retag의 cp 실패-계속을 재현(결함 재현) | 결정 2026-08-05(사용자, 외부 리뷰 수렴) — 성공 경로 바이트 불변, 장애 주입 케이스는 §9 이중 판정(완전 이전+rc≠0 또는 완전 이후+rc=0)으로 |
| 장부의 unreadable·headless 의미론 (v0.5.0 외부 리뷰 P0) | **absent ≠ unreadable, 귀속 불가 행은 사이드카 전체 무효.** 존재하는데 읽을 수 없는 장부(권한·디렉터리)는 validate가 `LEDGER-UNREADABLE`로 차단하고 scope가 명시하며 **모든 v1 폴백이 닫힌다**; id가 빈(headless) 행도 같은 결과 — 사라진 행이 누구의 최신 verdict였을지 알 수 없으므로. upgrade는 읽을 수 없는 장부 위에서 전 모드 거부 | 실측(2026-08-05): chmod-000 장부(마지막 verdict failed)가 validate rc 0 + scope 폴백 개방으로 **"빚 없음"**으로 읽혔고, 선행 TAB 하나로 failed 부채가 양쪽 표면에서 소멸했다(owed=0). v0.5.0의 headless 카운터는 소비자가 없는 죽은 코드였다. 기각: unreadable을 absent로 계속 접기(읽는 쪽의 fail-open — attest에서 고친 그 결함의 리더 판) | 결정 2026-08-05(사용자, v0.5.1 수렴) |
| validate의 장부 열 파서 | **정확 분리로 통일 — `readTabs` 은퇴.** bash `IFS=$'	' read` 재현(선행·연속 TAB 붕괴)은 원본이 삭제된 순간 근거를 잃었다 | 실측(2026-08-05): 빈 열 추가 → validate rc 0 / scope 격리, "named in scope and blocks in validate, never one without the other"가 한쪽만 성립. §11 위 행과 같은 수렴의 일부 | 결정 2026-08-05(사용자) |
| gaps kind 어휘의 기계 강제 | **`gaps.enum.kind`를 validate가 읽는다** — required 아래에서 양쪽 절 모두, 어휘 밖 kind는 `COMP-MALFORMED` | 선언-미독 스키마 키(schema 헤더가 경고하는 클래스). 오타 `[declraed]`가 Accepted에서 조용히 통과 — 존재하지 않는 kind에 대한 결정. Accepted의 `scope:`/`recheck:`/`as-of:` **필드**는 여전히 비강제(별도 결정 전까지) | 결정 2026-08-05(사용자, v0.5.1 수렴) |
| attest의 동시성 (v0.5.1 외부 리뷰 P0-1) | **장부 잠금 — 생성→꼬리검사→append→복구→미러가 한 임계구역.** 디렉터리 락(mkdir 원자성), 시효 회수, 유한 대기 후 명시 거부 **→ 시효 회수는 .2에서 대체됨**(6차 리뷰가 반증 — 아래 "잠금의 시효 회수" 행: 회수 제거) | 실측(2026-08-05, 시임으로 결정적): truncate-back은 보상 쓰기이고, 상호 배제 없는 보상 쓰기는 이웃의 rc-0 행을 지운다 — 기존 장부에선 행이 잘리고(TSV 멀쩡, validate 침묵), 신규 장부에선 unlink가 파일째 지웠다. 기각: 잠금 없이 truncate-back 폐기(부분 append 결함 재개방) · flock(node:fs에 없음) | 결정 2026-08-06(사용자, v0.5.2 수렴) |
| upgrade 백업 경로 (v0.5.1 외부 리뷰 P0-2) | **`mkdtempSync` — 항상 새 디렉터리.** 존재하는 경로를 반환할 수 없으므로 재사용 클래스가 제거된다 | 날짜+PID + `mkdirSync(recursive)`는 기존 디렉터리를 받았고, `bkup()`의 런-내 중복 제거가 낡은 파일을 이번 스냅샷으로 오인해 **롤백이 낡은 바이트를 복원하며 "byte-identical"을 보고**했다(드라이버 `--collide-bak`으로 red-first 증명). PID는 컨테이너에서 재활용된다 | 결정 2026-08-06(사용자) |
| 대체된 unknown verdict의 표면 (v0.5.1 외부 리뷰 P1-2) | **파서가 전 행의 verdict를 담고(oddVerdicts) scope가 명명한다.** winner는 그대로 — 복구된-장부 규칙 유지, 격리 아님 | validate가 차단하는 행이 장부를 서술하는 명령에서 보이지 않았다 — 두-리더 갈림의 이력(verdict) 축 | 결정 2026-08-06(사용자) |
| gaps 문법의 잔여 3종 + SCH_KEYS (v0.5.1 외부 리뷰 P1-3) | **브래킷 필수 · kind는 enum과 정확 일치(하나씩) · sentinel은 null · `gaps.*` 키 SCH_KEYS 편입** | `- no-kind`·`- []`·`- [declared\|reference]` 전부 통과했었다(각각 비강제·sentinel ''·inList 파이프-부분문자열). Accepted의 **필드**(scope:/recheck:/as-of:) 강제는 여전히 별도 결정 | 결정 2026-08-06(사용자) |
| 잠금의 시효 회수 (6차 리뷰 P0-1) | **자동 회수 제거 — 잠금은 사람이 지우기 전엔 잠금.** 잔여 잠금은 유한 대기 후 경로 명명 거부, 크래시의 비용은 사람이 그 경로와 내용물을 지우는 것(소유권 마크가 안에 있어 빈-디렉터리 제거로는 안 된다 — .5에서 문구·픽스처 정정) | 실측: 10초 시효가 **살아있는 보유자**(13s 보유)에게서 잠금을 훔쳐 이웃 rc-0 행이 롤백에 잘렸다. age로는 시체와 SIGSTOP·절전·느린 디스크를 구분할 수 없고 하트비트는 없다. 기각: 임계 연장(어떤 값도 반증됨) · 소유권 토큰/rename 회수(살아있는 보유자를 훔치는 건 동일 — 회수가 없으면 토큰도 불필요) | 결정 2026-08-06(사용자, 6차 리뷰 수렴) |
| 장부 writer의 잠금 편입 (6차 리뷰 P0-2) | **lock.mjs 공용 — attest와 upgrade --apply 트랜잭션 전체가 같은 잠금** | 실측: upgrade --apply가 살아있는 락을 그대로 통과(rc 0·장부 기록·언급 0), attest 롤백의 unlink가 방금 주조된 legacy 행째 파일을 지웠다. 완전 교차는 일관된 광산에서 구성 불가 — 쌍별 고정 | 결정 2026-08-06(사용자, 6차 리뷰 수렴) |
| gaps.sections 값 판독 (6차 리뷰 P1) | **절 이름·메시지 전부 스키마 값 유래; 두 멤버 아니면 SCHEMA-UNREADABLE.** gaps CLI도 같은 이름·any-level(sectionAll) | 로스터 편입(.1)은 키 삭제만 잡고 값은 안 읽었다 — 실측: Pending\|Waived 스키마에서 # Open 통과·# Pending 차단의 역전. CLI는 'Accepted' 하드코딩+h1/h2 한정으로 validate와 갈렸다 | 결정 2026-08-06(사용자, 6차 리뷰 수렴) |
| 이중 kind (6차 리뷰 P1) | **두 번째 브래킷이 kind 단어일 때만 차단** — 브래킷 인용문(`[계약서 3조]`)은 본문 | 첫 브래킷만 판정해 `- [declared] [reference]`가 rc 0(실측). 기각: 모든 후속 브래킷 차단(합법 인용 오탐) | 결정 2026-08-06(사용자, 6차 리뷰 수렴) |
| 태그 게이트 강화 (6차 리뷰 P1) | **leading-zero 금지 정규식 + 태그는 CHANGELOG `**vX.Y.Z — ` 클레임 줄에 존재해야 발행** | `v01.2.3` 통과(실측)·`v9.9.9`가 현 번들 발행 가능이던 구조. 기존 기록에 대한 단언이라 제2 진실원 아님. 태그→번들-날짜 매핑은 여전히 echo(규약 부재) | 결정 2026-08-06(사용자, 6차 리뷰 수렴) |
| 찢긴 장부 주석 (6차 리뷰 저우선) | **미종결 비어있지-않은 최종 줄은 주석이어도 malformed** — validate 명명 + 파서 파일-수준 계수(두 리더 한 답) | 실측: validate rc 0·scope rc 0·무언급. 주석은 증거가 될 수 없지만("#"으로 시작하는 행은 없다) 찢긴 쓰기의 흔적이다 | 결정 2026-08-06(사용자, 6차 리뷰 수렴) |
| placeholder kind와 실재 본문 (.2 콜드 리뷰) | **본문이 실재하는 placeholder-kind 불릿은 엔트리 — kind가 어휘 위반으로 차단. 순수 스텁(본문까지 placeholder)만 소음** | remainder-decides(v0.3.x)가 열어두던 마지막 형태: `- [<kind>] [declared] x — r`이 무진단 rc 0(실측) — 소음 분기가 kind 분기 앞이라 어휘·이중-kind 검사 모두 우회. 초기화 스텁의 green은 유지. 함께: `gaps.sections` 동일-멤버 로스터 SCHEMA-UNREADABLE, scope headless 문구 정직화 | 결정 2026-08-06(사용자, .2 콜드 리뷰 수렴) |
| placeholder의 continuation 실재화 (7차 리뷰 P1-1) | **연속줄이 불릿을 실재화하면 보류된 placeholder kind가 같은 어휘 규칙으로 판정된다** — 브래킷 단어가 소음 플래그와 동행 | 위 결정의 연속줄 판: 소음 보류 후 실제 내용의 continuation이 계수만 하고 판정을 안 했다(실측: validate rc 0 + CLI "records 0" — §11 위 결정문과 직접 충돌) | 결정 2026-08-06(사용자, 7차 리뷰 수렴) |
| upgrade --apply의 preflight 시점 (7차 리뷰 P1-2) | **잠금 취득 후 캐시 무효화 + config 스냅샷 재구성 + 전체 preflight 재실행** — 진 쪽은 "nothing to do" rc 0 | 계획이 잠금 앞에서 계산돼 동시 apply의 패자가 낡은 계획을 이행 끝난 광산에 재적용(실측: 둘 다 "applied 4", 백업 2개 — 두 번째 MANIFEST가 복구점을 거짓 서술). m.cfg가 열림-시점 스냅샷이라 재구성 없이는 재스캔도 낡다 | 결정 2026-08-06(사용자, 7차 리뷰 수렴) |
| 태그 클레임의 범위 (7차 리뷰 P1-3) | **최상단 `## <번들>` 절 안, 행 시작 `**vX.Y.Z — `만 인정** | 과거 클레임 줄이 영구 잔존해 whole-file grep이 옛 태그의 오재지정을 통과시켰다(실측: v0.5.2 트리에서 v0.5.1 술어 통과). 옛 태그 재실행은 제 트리를 체크아웃하므로 정합 | 결정 2026-08-06(사용자, 7차 리뷰 수렴) |
| 잠금 소유권 마크 (7차 리뷰 저우선) | **mkdir 직후 nonce 기록, release는 마크 일치 시에만 제거 — 회수 채널 아님**(잔여 락은 마크 유무·소유 불문 전원 거부, 사람 몫) | 사람이 살아있는 락을 지운 뒤 새 writer가 잡으면 이전 보유자의 release가 새 락을 지웠다(red 실측: FOREIGN-REMOVED). 이중 인간-오류 게이트지만 무회수 설계와 충돌 없이 구조적으로 닫힘 | 결정 2026-08-06(사용자, 7차 리뷰 수렴) |
| **단일 writer 계약** (9차 리뷰 P0) | **광산 하나에 변형 명령 하나 — 선언하고 강제한다.** FORMATS·README·WORKFLOW·전 스킬에 계약 명시(보증은 이 계약 아래서만 성립, 복구는 재실행이 아니라 재검토·재검증) + dispatcher가 변형 명령에 `.weavedoc/mine.lock`을 **명령의 광산-판단 이전**에 취득(root 해소 openMine 1회는 잠금 앞 — 10차 정정 반영), 둘째는 **거부**(대기 아님). 읽기 전용 명령과 읽기 전용 모드는 비게이트 | 실측: upgrade 트랜잭션 중 seal-review 투입 → 광산이 validate rc 1. `retag↔retag`·`upgrade↔seal-review`는 **잠금 이전부터 있던 데이터 경합**이고(잠금이 만든 건 잘못된 기대), 명시된 계약은 없었다 — WORKFLOW의 "main session"은 암시. 진입 취득이라 명령별 재배치·reentrant 불필요. 기각: 광산 mutation lock의 완전 구현(=명령마다 판단 재배치, 2~3라운드 + 새 기계) → **v0.6 항목**. 한계 기록: 에이전트의 직접 편집·공유 드라이브 두 체크아웃은 잠금 밖 · 크래시 잔재가 이제 전 쓰기 명령을 막음 | 결정 2026-08-06(사용자, 9차 리뷰 수렴 — B+) |
| gaps 문법의 3축 (9차 리뷰 P1) | **kind 슬롯은 전체를 strip해 판정**(접두사 아님) · **항목은 0열에서 열린다**(들여쓴 불릿은 연속줄) · **등록부 밖 항목 차단**(제3 절의 산문은 자유) | 슬롯을 접두사로만 봐서 `- [{kind} real-content]`가 소음으로 통과(본문이 있으면 .4가 이미 차단 — 열린 건 슬롯만 있는 형태) · 부모 없는 들여쓴 불릿이 결정으로 통과하고 정상 sub-bullet은 오차단 · 제3 절의 gap은 아무도 안 셈. 슬롯이 통째로 한 placeholder 그룹이면 여전히 스텁(값 전체를 보는 기존 판정과 정합) | 결정 2026-08-06(사용자, 9차 리뷰) |
| v0.6 후보 (9차 리뷰·콜드 리뷰에서 기록만) | **① 광산 mutation lock 완전판** — 현 dispatcher 진입 잠금과의 관계: 진입 잠금은 CLI 명령 **사이**의 배제이고, 완전판은 retag·seal-review·consecrate·reindex의 **판단(읽기)까지** 잠금 안으로 재배치하는 것(attest·upgrade에 했던 작업의 반복, 2~3라운드). 진입 잠금이 선 지금 잔여 가치는 드라이버-직접-호출 경로와 크래시-복구 중 상태뿐 ~~② `status`↔`validate`의 `Human queue` 깊이 3~6 갈림~~ — **해소 확인(2026-08-06 실측)**: 깊이 3·4·5·6 전부 validate와 status가 같은 수를 낸다(v0.5.4의 sectionAll 깊이≤6 통일로 함께 닫혔고, 목록만 낡았다) ③ `attest`의 `Verified units` 정규식 상한·공백 부재 ④ validate와 gaps CLI의 **단일 gaps state parser** — **등록부 리더는 v0.5.6~v0.5.8에서 통일됨**(validate·`status --open`·`weavedoc gaps` 전부 `scanRegister`). 남은 것은 **그 앞단의 전처리**다 — nocomment·defence를 각자 부르므로 재현체(주석-합성 opener)는 그대로다(재현체: 12차 리뷰의 주석-합성 opener — nocomment가 원문에 없던 0열 opener를 만들어 뒤의 실제 미아가 rc 0으로 숨는다, 실측) | 전부 실측된 선행 결함이고, 전부 **새 기계**를 요구한다 — 이번 라운드는 계약 선언 + 진입 승인으로 기대와 현실을 맞추는 데까지다. ①은 2.4 조건(CI 병렬·공유 드라이브·상시 병렬 세션·git 밖 광산) 중 하나가 사실이 되면 즉시 승격 | 기록 2026-08-06(사용자, 9차 리뷰 B+ 결정과 함께) |
| parser/state model 재설계 (v0.6 후보 ③·④ 해소) | **파일마다 lexical scan 1회 + 구조 pass 1회 + artifact별 typed policy.** frontmatter/comment/fence/heading은 배타적 context와 source offset을 갖고, Human queue·questions·gaps는 syntax/materialisation/body/slot/hierarchy 축을 독립 상태로 보존한다. review는 shape-free detection을 유지하되 mutation authority를 명백한 legacy record token으로 좁힌다. Verified units·coverage·writer도 같은 heading/source node를 쓴다 | 사례별 `nocomment`/`defence`/정규식을 덧대며 발생한 조합 폭발을 상태공간으로 바꾼다. `sections.mjs`·`review.mjs` 삭제, 순수 Cartesian property matrix + black-box writer postcondition으로 강제. 상세 계약은 `.weavedoc/PARSER-MODEL.md`. 기존 후보 ③(`attest` Verified units reader)과 ④(gaps 전처리/reader)는 해소; 후보 ① mutation lock 완전판만 남음 | 구현 2026-08-08(미발행 번들 `2026-08-08.1`) |
| **릴리스 기준** (11차 리뷰 후 결정) | **태그를 막는 결함은 두 종류뿐: (a) 데이터를 파괴하는 것 (b) 정상 사용 경로에서 잘못 차단하거나 잘못 통과시키는 것.** 그 외 발견(병적 입력·opt-in 기능의 좁은 경로·문구)은 known-issue로 기록하고 다음 릴리스로 보낸다. 외부 리뷰는 계속 받되 태그의 인질이 아니다 | 11라운드의 실측 궤적: 데이터 파괴(5~6차) → 동시성(7~9차) → 정합성(10차) → 병적 입력(11차)로 발동 조건이 단조 축소. 리뷰가 빈손으로 돌아오는 일은 없으므로 "리뷰 클린 = 태그"는 종료 조건이 없는 루프였다. 기각: 무기준 반복(이 표가 그 비용의 기록이다) | 결정 2026-08-06(사용자 — "기준 채택 + 마지막 1라운드") |
| 11차 리뷰 반영 (마지막 라운드) | **펜스 판정을 gaps의 네 리더 전부가 공유**(sections.mjs `defence` — opener는 남기고 내용·closer를 블랭크, 미종결 보고; backtick opener의 info에 `` ` `` 불가 = CommonMark) · **동일 모드 중복도 usage 오류** · §11 761행 자기모순·regress 주석 3건·CHANGELOG .6 과장 문장 정정 | 실측: 펜스 안 가짜 등록부 rc 0(4 리더 중 1만 펜스 인지 — 10차 수정이 만든 갈림) · 펜스 예시의 정상 파일 오차단 · `` ```foo`bar ``가 펜스로 읽혀 실제 미아 은폐. opener를 남기는 이유: 등록부 절 안의 펜스는 문법 위반으로 **계속 차단**돼야 한다(fail-closed 유지) | 결정 2026-08-06(사용자, 11차 리뷰 수렴) |
| 10차 리뷰 반영 (태그 보류 지적) | **resume 키에 저장소-상대 전체 경로**(basename이 아니라 — 공용 key_paths 함수, KEY 앞 정의 + 공허 가드) · **펜스는 문자·개수·들여쓰기(≤3)를 기억**(미종결은 `<!--` 판정) · **upgrade 모드 둘 이상 = usage 오류**(last-wins는 게이트가 공유 못 하는 숨은 규칙) · "openMine 읽기 전 잠금" 서술을 실제 순서로 정정 · held-lock 케이스 writer 6종 전수 | 실측: 디렉터리 간 동일-basename 이동에 키 불변 → --resume이 0회 실행으로 430 재생 · 4칸-들여쓴 가짜 펜스가 진짜 항목을 삼키고 4-tick 안 3-tick이 오차단 · `--apply --check`가 read-only인데 잠금 거부. 기각: 게이트가 last-wins 복제(숨은 규칙의 둘째 사본) | 결정 2026-08-06(사용자, 10차 리뷰 수렴) |
| 9차 P2 | 제목 깊이 상한 6을 **Human queue 자체 walker에도**(.4가 sectionAll에만 넣어 status·validate 재분기) · `--resume` 키에 **파일 경로** 편입 | 해시만 담고 이름을 버려, 정렬 순서를 보존하는 rename이 키를 그대로 두고 과거 성공을 재사용시켰다(실측) | 결정 2026-08-06(사용자, 9차 리뷰) |
| 잠금 경계의 클래스 (8차 리뷰 P1-1·P1-2) | **판단은 전부 잠금 안.** upgrade: preflight를 함수 하나로 뽑아 `--apply`가 잠금 아래서 한 번 호출(중복 삭제) · attest: id 해소·상태·다이제스트를 잠금 안으로, 인자 문법만 밖 · **클래스 가드는 경과 시간으로 측정**(계측 없음) · 예외는 **잠금 경로 자체의 전제**(truths/ 존재)뿐 — 양 명령이 같은 철자로 잠금 앞에서 검사. 잠금이 쥐어져 있으면 이미-이행·잘못된/미래 버전·읽을 수 없는/id 없는 장부 **다섯 거부가 전부 잠금 거부로 대체**된다(의도된 귀결) | 실측: 이행된 광산에서 두 번째 apply가 0초에 "nothing to do" rc 0(승자는 롤백) · 대기 중 바뀐 truth가 사라진 바이트에 대해 verified로 기록되고 착지 즉시 stale. 세 라운드가 같은 경계를 한 칸씩 좁혀왔으므로 **사례가 아니라 클래스**로 닫는다 | 결정 2026-08-06(사용자, 8차 리뷰 — 마지막 라운드) |
| gaps 문법의 클래스 (8차 리뷰 P1-3) | **미폐합 브래킷은 malformed**(placeholder 판정보다 먼저) + **진리표 13칸을 케이스로 고정** | `- [{kind}`·`- [<kind>`가 strip()에 지워져 소음으로 통과(실측 rc 0). 이 표의 칸을 라운드마다 하나씩 닫아왔다 — 표 전체를 고정해 다음 칸을 없앤다 | 결정 2026-08-06(사용자, 8차 리뷰) |
| 태그 클레임의 유일성 (8차 리뷰 P1-4) | **최상단 절의 앵커된 클레임을 전부 추출 → 개수 1 + 태그 일치** | "하나라도 맞으면"이라 진짜 클레임 옆에 과거 태그 줄을 밀어 넣으면 옛 태그가 통과(실측) | 결정 2026-08-06(사용자, 8차 리뷰) |
| 8차 P2 묶음 | **거부 문구를 실제 복구 절차와 일치**(owner 포함 크래시 픽스처로 케이스 교체) · dead ledger에서 superseded 줄 억제 · **제목 깊이 상한 6을 sectionAll도 지킴** · IMPROVEMENT_PLAN 머리 정정 · **릴리스 증거 블록**(tag·bundle·schema·fingerprint·node·manifest sha)을 노트에 동봉 | nonce 도입이 만든 문서-현실 갈림(rmdir이 ENOTEMPTY로 실패) — **픽스처가 현실보다 순했던 것**이 원인 · 무효 선언 옆에서 승자를 history라 서술 · `####### Accepted`가 validate엔 malformed·CLI엔 1건 · 만료되는 잡 로그에만 있던 신원 정보 | 결정 2026-08-06(사용자, 8차 리뷰) |
| 7차 저우선 묶음 | **대기 시계 performance.now(단조) · oddVerdicts id별 전체 단어('·' 병기) · KEY에 README/CHANGELOG/FORMATS · 실교차 상주 회귀("구성 불가" 주석 정정)** | 벽시계 역행이 유한 대기를 늘림 · 첫-단어만 담아 validate/scope 계수 갈림 · doccheck 케이스의 입력 미키잉 · pristine이 canonical-id v1 광산이라 attest 가동(주석의 과잉일반화 실측 반증) | 결정 2026-08-06(사용자, 7차 리뷰 수렴) |
...
**고치지 않았다 — 그리고 이제 고칠 대상이 없다**: bash 판은 번들 2026-08-05.3에서 삭제됐다. 이 항목은 **삭제된 런타임의 결함으로 닫힌다**. Node 판에는 이 자리가 없다(fs로 읽고 포크하지 않는다). 기록을 남기는 이유는 결함 자체가 아니라 **기제**다 — 포크 실패를 확인 없이 캐시하면 그 파일이 빈 문서가 되고 이후 모든 검사가 "문제 없음"을 낸다는 fail-open 모양은, 외부 프로세스를 부르는 코드를 다시 쓰게 되면 그대로 재발한다.

### known-issue 장부 (현재 열린 것 — 2026-08-22 개설)

§11의 릴리스 기준은 "그 밖의 발견은 known-issue로 기록하고 다음 릴리스로 보낸다"고 정했지만, **기록되는 한 곳을 정하지 않았다.** 실제 관행은 릴리스마다 CHANGELOG 항목 안에 "Known issues(이 릴리스가 새로 남기는 것만)"를 적고 나중에 취소선으로 지우는 것이었고, 그래서 "오늘 열려 있는 게 뭔가"는 번들 항목 N개를 걸어가며 취소선을 적용해야만 나왔다. 그 비용은 실측됐다 — 번들 2026-08-06의 전수 점검이 **이미 닫혔는데 미결로 보이던 것 3건**을 찾았다(§CHANGELOG 604행). 자매 프로젝트 GroveSpec이 같은 기준을 채택하며 함께 만든 것이 이 장부다.

**계약**
- 릴리스별 CHANGELOG의 "Known issues" 절은 **이력**이다 — 그 릴리스가 새로 남긴 것. 그대로 둔다.
- 이 절은 **현재 상태**다 — 오늘 열려 있는 것 전부, 한 곳에.
- 항목이 들어오는 조건: §11 기준이 (a)도 (b)도 아니라고 분류한 것.
- 항목이 나가는 조건은 **둘뿐**: 고쳤거나, **실측으로 승격**됐거나. 설계 완결성은 사유가 아니다.
- 다음 릴리스의 첫 일은 이 목록을 한 번 훑는 것이다. 그것이 이 절이 존재하는 이유다.

**출처 표기**: `실측` = 직접 재현·측정함. 개설 시점의 `전사`(릴리스 노트 전사, 미재측정) 10건은 **2026-08-22 스윕에서 전부 재봤고**, 아래는 그 결과다 — 이제 모든 행이 `실측`이다.

**개설 당일 스윕 (2026-08-22) — 전사 10건 중 4건이 이미 닫혀 있었다.** 장부가 존재하는 이유를 첫날 스스로 증명한 셈이다(2026-08-06 점검이 같은 비율을 찾았던 것과 같은 꼴).

- **닫힘 — 고아 continuation이 HQ·questions에서 조용히 버려짐**(v0.5.7 전사). 재현 결과 반대다: stub + 빈 줄 + continuation의 내용이 부모 항목에 **붙어서** `status --open`에 나온다(questions·HQ 양쪽). parser/state model 재설계가 덮은 것으로 보인다.
- **닫힘 — `\v` 들여쓰기 등 더 깊은 공백 병리**(v0.5.7 전사). 파킹 사유는 "조용함"이었는데 지금은 **양쪽 모드에서 명명된다**: `completeness: off`에서도 `status --open`이 "gaps.md '# Open' holds a line the register grammar cannot read, so nothing after it is listed"를 찍고, `required`에서는 `COMP-MALFORMED`가 **그 줄을 인용하며** 차단한다. 읽히지 않는다는 사실 자체는 문서화된 문법(항목은 컬럼0)이라 결함이 아니다. *(첫 측정에서 `grep`이 경고 줄을 잘라 "조용하다"고 잘못 읽었다 — 필터를 걷고 다시 쟀다.)*
- **닫힘 — 주석-펜스 중첩이 없던 opener를 합성해 뒤의 미아를 rc 0으로 숨김**(v0.5.10 전사). `<!-- x -->```` 로 재현했으나 숨지 않는다: `off`에서 경고 + `required`에서 `COMP-MALFORMED` **rc 1**. 합성된 줄이 인용돼 나온다.
- **닫힘 — validate는 latin1, CLI는 utf8로 읽는 선행 갈림**(v0.5.10 전사). 현재 `cmd-validate`·`cmd-status`·`cmd-gaps`와 공유 스캐너 `markdown-scan` 전부 latin1이다. 갈림이 없다. (`read.mjs`의 utf8 기본값은 `fmLoadBytes`를 바이트 쌍으로 갖는 의도된 2도메인 설계이지 이 갈림이 아니다.)

**서술이 틀렸던 1건 — held-lock과 복수-모드 오류의 선행 순서**(v0.5.10 전사). 재보니 **막을 usage 오류가 없다**: `cmdUpgrade`는 *모르는* 인자만 거부하고 모드 개수는 세지 않으며, dispatcher에도 검사가 없고 회귀 케이스도 없다. 10차 리뷰가 결정으로 기록한 "upgrade 모드 둘 이상 = usage 오류"가 구현되지 않았다. 그 아래에서 실측된 것은 known-issue가 아니라 **(b)급 결함**이므로 이 장부가 아니라 수리 대상이다 — 아래 별도 절.

| 항목 | 왜 강제 수리가 아닌가 | 승격 조건 | 출처 |
|---|---|---|---|
| BOM이 붙은 광산 파일은 **읽히지 않고 거부된다**(진단이 원인은 지목한다 — 번들 `.33`) | 전 리더가 같은 답을 내고 fail-closed다. 받아들이려면 `splitLines`가 두 바이트 도메인 모두에서 BOM을 벗겨야 하는데, 그러면 utf8 리더와 latin1 라이터가 파일의 바이트에 대해 갈라진다(`seal-review`는 `splitLines` 결과를 다시 이어 붙여 쓴다 — 사용자 파일에서 BOM이 조용히 사라지고 digest가 바뀐다). 드문 입력을 위해 split-judgment 클래스를 사는 거래 | 사용자가 다시 저장할 수 없는 BOM 파일이 실제 광산에 들어옴 | 실측 2026-08-22 |
| 접힌 줄 아래 하위 불릿이 오면 한 줄에 `- [kind]` 토큰이 둘 보임 (`- [declared] - [reference] 하위 불릿`) | 표시 형태만의 문제 — 항목 수·차단 판정은 옳다(과다 표시 방향) | 이 표시가 실제 오판정으로 이어짐 | 실측 2026-08-22 |
| 읽기 불가 `verify.md`에서 plain `status`는 `human queue: 0`을 찍는다(`--open`은 "cannot be read" 경고) | 두 모드의 출력 계약이 달라 `status`의 계약 변경이 선행해야 한다. `--open`이 같은 상태를 명명하므로 정보가 사라지진 않는다 | 두 명령의 갈림이 실제로 사용자를 오도한 사례 | 실측 2026-08-22 |
| `questions.md`의 들여쓴 `- [open]`은 목록에 오르지 않는다 | **문서화된 문법**(FORMATS: 항목은 컬럼0에서 열리고 들여쓴 줄은 continuation) — hq 카운터의 들여쓰기 관용과 비대칭이지만 항목 문법이 우선이라는 의도된 순위 | 사용자가 쓴 들여쓴 항목이 실제로 유실됨 | 실측 2026-08-22 |
| 중복 `# Open` 아래에서 `status --open`이 두 절의 항목을 다 나열해 과다 보고 | 안전 방향(과소가 아니라 과다)이고, `required`에서 `COMP-MALFORMED`가 차단하며 `weavedoc gaps`는 "the tally below is incomplete"를 찍는다 | 과다 보고가 결정을 잘못 이끈 사례 (안전 방향이므로 승격에는 반대 증거가 필요) | 실측 2026-08-22 |
| shellcheck 스타일-레벨 잔여 | **설계상 이연** — CI가 `--severity=error`만 차단하고 style 전수는 §9 릴리스 조건으로 미뤄져 있다 | style 경고가 실제 결함을 가린 사례 | 실측 2026-08-22 (CI 워크플로 확인; 이 머신엔 shellcheck 미설치라 로컬 실행은 못 함) |

### 수리됨 — `upgrade`가 상충하는 모드를 받고 썼다 (2026-08-22 실측 → 번들 `.34` 수리)

`weavedoc upgrade --check --apply`가 **썼다.** 사용자가 "쓰지 마라"(`--check`)와 "써라"(`--apply`)를 한 줄에 적었는데 명령은 조용히 후자를 골랐다 — usage 오류도, `--check`가 무시됐다는 말도 없이 rc 0. 실측(v3 광산에서 intake 장부를 지워 backfill 대상을 만든 뒤): `materials/intake-ledger.tsv`에 행이 실제로 기록됐다.

§11 기준으로 **(b) 정상 사용 경로의 오통과**. argv 게이트(WD-CLI-001)는 *모르는* 플래그만 보므로 **아는 플래그 둘이 상충할 때**가 사각이었다. 10차 리뷰가 이 규칙을 결정으로 적었으나 구현이 따라가지 않았고, 그래서 파킹 항목("held-lock과 복수-모드 오류의 선행 순서")은 **존재하지 않는 거부 둘의 순서**를 서술하고 있었다 — 장부를 훑지 않았으면 그 오서술이 계속 남았을 자리다.

**수리**: 모드 토큰이 둘 이상이면 exit 2(중복 토큰 포함). 잠금과의 순서는 by-design 그대로 — 쥐어진 lock은 dispatcher에서 rc 1, 이 usage 거부는 rc 2, 둘 다 무쓰기. 회귀 3건이 양방향 변이로 확인됐고, 핵심 케이스는 문구가 아니라 **파일의 부재**를 단언한다.
