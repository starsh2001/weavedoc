...
| v0.6 후보 (9차 리뷰·콜드 리뷰에서 기록만) | **① 광산 mutation lock 완전판** — 현 dispatcher 진입 잠금과의 관계: 진입 잠금은 CLI 명령 **사이**의 배제이고, 완전판은 retag·seal-review·consecrate·reindex의 **판단(읽기)까지** 잠금 안으로 재배치하는 것(attest·upgrade에 했던 작업의 반복, 2~3라운드). 진입 잠금이 선 지금 잔여 가치는 드라이버-직접-호출 경로와 크래시-복구 중 상태뿐 ② `status`↔`validate`의 `Human queue` 깊이 3~6 갈림(필수-절 리더가 `##`까지만) ③ `attest`의 `Verified units` 정규식 상한·공백 부재 ④ validate와 gaps CLI가 **공유하는 단일 gaps state parser**(재현체: 12차 리뷰의 주석-합성 opener — nocomment가 원문에 없던 0열 opener를 만들어 뒤의 실제 미아가 rc 0으로 숨는다, 실측) | 전부 실측된 선행 결함이고, 전부 **새 기계**를 요구한다 — 이번 라운드는 계약 선언 + 진입 승인으로 기대와 현실을 맞추는 데까지다. ①은 2.4 조건(CI 병렬·공유 드라이브·상시 병렬 세션·git 밖 광산) 중 하나가 사실이 되면 즉시 승격 | 기록 2026-08-06(사용자, 9차 리뷰 B+ 결정과 함께) |
