...
**출처 표기**: `실측` = 이번에 직접 재현·측정함. `전사` = 릴리스 노트에서 옮겨 적었고 **다시 재보지는 않았다** — 이미 닫혔을 수 있고, 그것을 확인하는 것이 다음 스윕의 몫이다. 재보지 않은 것을 재본 것처럼 적지 않는 것이 이 장부가 대체하려는 실패의 반대다.
...
| 고아 continuation(stub + 빈 줄 + continuation)이 gaps에서만 명명되고 HQ·questions에서는 조용히 버려짐 | "못 읽는 것은 명명한다"가 세 장부 중 한 곳에만 적용된 기존 클래스 | 버려진 항목이 실사용에서 결정을 숨김 | 전사 (v0.5.7) |
| `\v` 들여쓰기 등 더 깊은 공백 병리 | 병적 입력 — §11 기준 밖 | 실사용 입력에서 발화 | 전사 (v0.5.7) |
| 접힌 줄 아래 하위 불릿이 오면 한 줄에 `- [kind]` 토큰이 둘 보임 | 드문 형태, 과다 표시 방향 | 오판정으로 이어짐 | 전사 (v0.5.8) |
| 읽기 불가 `verify.md`에서 plain `status`는 `human queue: 0`(`--open`은 경고함) | `status`의 출력 계약 변경이라 별도 결정 | 두 명령의 갈림이 사용자를 오도한 사례 | 전사 (v0.5.9) |
| `questions.md`의 들여쓴 `- [open]`은 목록에 오르지 않음(컬럼0 항목 문법) | 항목 문법이 우선 — hq 카운터의 들여쓰기 관용과 비대칭이지만 의도된 우선순위 | 사용자가 쓴 들여쓴 항목이 실제로 유실됨 | 전사 (v0.5.9) |
| 중복 `# Open` 아래에서 validate가 차단만 하고 세지 않아 목록이 과다 보고됨 | 안전 방향(과소가 아니라 과다) | — (안전 방향이므로 승격은 반대 증거가 필요) | 전사 (v0.5.9) |
| 주석-펜스 중첩에서 nocomment 우선이 원문에 없던 opener를 **합성**해 뒤의 미아를 rc 0으로 숨김 | 병적 입력·정상 경로 무영향 — 12차 리뷰가 태그 후 실측하고 기준 미달로 분류 | 정상 작성 경로에서 재현 | 전사 (v0.5.10) |
| validate는 latin1, CLI는 utf8로 읽는 선행 갈림 | ASCII 펜스·절 이름엔 무영향 | 비ASCII 절 이름에서 두 명령이 다른 답을 냄 | 전사 (v0.5.10) |
| shellcheck 스타일-레벨 잔여 | §9 릴리스 조건에서 이미 이연 | — | 전사 (v0.5.10) |
| held-lock과 복수-모드 오류가 겹치면 잠금 거부(rc 1)가 usage(rc 2)에 선행 | 어느 쪽이든 무쓰기 거부 — by-design으로 기록됨 | — | 전사 (v0.5.10) |
