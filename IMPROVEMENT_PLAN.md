...
> 상태(2026-08-07 갱신): **개선 열차 완료 — v0.5.0 발행, bash 런타임 삭제, 런타임은 Node 하나.** Phase 0~5 전부 완료; 이후 이력은 이 줄이 아니라 **CHANGELOG.md(번들 라벨별)와 §11 결정표**가 정본이다. 이 줄은 v0.3.6까지의 단계 이력을 담고 있었고, 그 전문은 git 이력(`git log -p -- IMPROVEMENT_PLAN.md`)에 보존돼 있다. 현행: **v0.5.21 발행(2026-08-07, 번들 `2026-08-07.10`) — 장부의 펜스 실명(fenced 예시가 대기 항목이던 것)·빈 줄이 구조를 파괴하던 것·configured 경로 전수(inbox 누락)·gaps 계약 정합.** 그 앞: v0.5.20(번들 `2026-08-07.9`, 실제 발행분 — v0.5.18은 3-OS red, v0.5.19는 릴리스 게이트 거부로 **둘 다 미발행**), v0.5.18의 내용(장부 구조 판정 공유 `hq-ledger.mjs` + downstream CRLF/EOL·빈 디렉터리 marker·pathspec 격리·매니페스트 fail-closed). 그 앞: v0.5.17(부모를 들여쓰기로 판정 + git 환경 전면 격리), v0.5.16(테스트가 채점하는 트리를 건드리지 않음), v0.5.15(디렉터리 통째 해시로 KEY seal 완결), v0.5.14(인덱스가 실제로 키에 들어감), v0.5.13(seal 도입 + 릴리스 멱등). 그 앞: v0.5.12(하네스 속도 30분→7분), v0.5.11(태그 공백 한 철자), v0.5.10(placeholder×continuation 조합, 접기=표시/판정=항목줄), v0.5.9(미결 목록 전수 점검 + gaps CLI 펜스 침묵). 그 앞: v0.5.8이 gaps 등록부 리더를 하나로 통일, v0.5.7이 세 장부 continuation 표시 통일. 그 앞: v0.5.6이 v0.5.5의 `status --open` above-bar 결함(등록부 리더 갈림)을 수정. 그 앞: v0.5.4(36841c3) 리뷰-수정 루프 종료 → v0.5.5(eecb697) Surface-don't-point 규칙 + `status --open` 신설. 릴리스 기준은 §11 결정(태그를 막는 결함은 데이터 파괴·정상 경로 오차단/오통과뿐 — §9의 전체 ShellCheck 통과 등 품질 조건은 백로그이지 태그 게이트가 아니다); 이후 리뷰 발견은 그 기준으로 분류부터 한다. 다음 갈림길: v0.6 후보(§11) 또는 실사용 전환.
>
