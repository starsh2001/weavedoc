...
> 상태(2026-08-05 갱신): **개선 열차 완료 — v0.5.0 발행, bash 런타임 삭제, 런타임은 Node 하나.** Phase 0~5 전부 완료; 이후 이력은 이 줄이 아니라 **CHANGELOG.md(번들 라벨별)와 §11 결정표**가 정본이다. 이 줄은 v0.3.6까지의 단계 이력을 담고 있었고, 그 전문은 git 이력(`git log -p -- IMPROVEMENT_PLAN.md`)에 보존돼 있다. 현행: **v0.5.4 발행 완료(36841c3, 2026-08-06) — 리뷰-수정 루프 종료.** 릴리스 기준은 §11 결정(태그를 막는 결함은 데이터 파괴·정상 경로 오차단/오통과뿐 — §9의 전체 ShellCheck 통과 등 품질 조건은 백로그이지 태그 게이트가 아니다); 이후 리뷰 발견은 그 기준으로 분류부터 한다. 다음 갈림길: v0.6 후보(§11) 또는 실사용 전환.
>
