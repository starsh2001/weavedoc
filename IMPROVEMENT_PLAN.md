...
| v0.6 후보 (9차 리뷰·콜드 리뷰에서 기록만) | **① 광산 mutation lock 완전판**(retag·seal-review·consecrate·reindex의 판단까지 잠금 안으로 — 명령마다 재배치, 2~3라운드) ② `status`↔`validate`의 `Human queue` 깊이 3~6 갈림(필수-절 리더가 `##`까지만) ③ `attest`의 `Verified units` 정규식 상한·공백 부재 ④ validate와 gaps CLI가 **공유하는 단일 gaps state parser** | 전부 실측된 선행 결함이고, 전부 **새 기계**를 요구한다 — 이번 라운드는 계약 선언 + 진입 승인으로 기대와 현실을 맞추는 데까지다. ①은 2.4 조건(CI 병렬·공유 드라이브·상시 병렬 세션·git 밖 광산) 중 하나가 사실이 되면 즉시 승격 | 기록 2026-08-06(사용자, 9차 리뷰 B+ 결정과 함께) |
