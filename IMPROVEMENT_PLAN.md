...
> 상태(2026-08-05 갱신): **개선 열차 완료 — v0.5.0 발행, bash 런타임 삭제, 런타임은 Node 하나.** Phase 0~5 전부 완료; 이후 이력은 이 줄이 아니라 **CHANGELOG.md(번들 라벨별)와 §11 결정표**가 정본이다. 이 줄은 v0.3.6까지의 단계 이력을 담고 있었고, 그 전문은 git 이력(`git log -p -- IMPROVEMENT_PLAN.md`)에 보존돼 있다. 현행 작업: v0.5.1(외부 리뷰 후속 — 장부 파서 완성·쓰기 내구성·문서 정합).
>
