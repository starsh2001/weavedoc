...
> 상태(2026-08-05 갱신): **개선 열차 완료 — v0.5.0 발행, bash 런타임 삭제, 런타임은 Node 하나.** Phase 0~5 전부 완료; 이후 이력은 이 줄이 아니라 **CHANGELOG.md(번들 라벨별)와 §11 결정표**가 정본이다. 이 줄은 v0.3.6까지의 단계 이력을 담고 있었고, 그 전문은 git 이력(`git log -p -- IMPROVEMENT_PLAN.md`)에 보존돼 있다. 현행 작업: v0.5.4(8차 외부 리뷰 후속, 이 사이클의 마지막 라운드 — 잠금 경계·gaps 문법·태그 게이트를 사례가 아니라 클래스로 닫음).
>
...
| 잠금의 시효 회수 (6차 리뷰 P0-1) | **자동 회수 제거 — 잠금은 사람이 지우기 전엔 잠금.** 잔여 잠금은 유한 대기 후 경로 명명 거부, 수동 rmdir이 크래시의 비용 | 실측: 10초 시효가 **살아있는 보유자**(13s 보유)에게서 잠금을 훔쳐 이웃 rc-0 행이 롤백에 잘렸다. age로는 시체와 SIGSTOP·절전·느린 디스크를 구분할 수 없고 하트비트는 없다. 기각: 임계 연장(어떤 값도 반증됨) · 소유권 토큰/rename 회수(살아있는 보유자를 훔치는 건 동일 — 회수가 없으면 토큰도 불필요) | 결정 2026-08-06(사용자, 6차 리뷰 수렴) |
